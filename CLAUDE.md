...
The backend introduces uses a remote server for writting to the database. It is spawn from the main process and the communication happens using ZMQ and protobuf as encoding.
This is transparent in Python and the developer doesn't see it but it is important that READ cursors ARE ONLY FOR READING AND DON'T WRITE DATA. For writing we use write cursors.
THIS LOGIC IS BEING IMPLEMENTED SO MIGHT BE FAULTY.
