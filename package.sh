...
if [[ $ARCH == 'x86_64' ]];then
  ...
  git checkout v${PYINSTALLER_VERSION}
  cd bootloader || exit 1
  ./waf all --target-arch "${ARCH}"
  cd ..
...
if [[ $? -ne 0 ]]; then
    if [[ "$ARCH" == 'arm64' ]] && [[ -z "${CI+x}" ]]; then
        read -p "There was an error during the sanity check, do you wish to continue y/N? " -n 1 -r
        if [[ ! $REPLY =~ ^[Yy]$ ]]
        then
            exit 1
        fi
...
if [[ "$PLATFORM" == "darwin" ]]; then
    ...
    mkdir "$WORKDIR/build"
    mv "rotkehlchen-backend-${ROTKEHLCHEN_VERSION}-macos.zip" "$WORKDIR/build"
    cd "$WORKDIR" || exit 1
fi
