...
.. http:post:: /api/(version)/blockchains/ETH/transactions
   ...
   **Example Request**:
   ...
   :reqjson list tx_hashes: The list of transaction hashes to request decoding for
   :reqjson bool async_query: Boolean denoting whether this is an asynchronous query or not
...
.. http:put:: /api/(version)/import
   ...
   **Example Request**:
   ...
   :reqjson str source: The source of the data to import. Valid values are ``"cointracking.info"``, ``"cryptocom"``, ``"blockfi-transactions"``, ``"blockfi-trades"``, ``"nexo"``,  ``"shapeshift-trades"``, ``"uphold"``.
