...
.. http:post:: /api/(version)/blockchains/transactions/
   ...
   Doing a POST on the blockchains transactions endpoint will query all transactions for all the tracked user addresses and save them to the DB. Caller can also specify a list of accounts to further filter the query.
   ...
   **Example Request**:
   ...
   :reqjson list[string] accounts: List of accounts to filter by. Each account contains an ``address`` field which is a blockchain address and an optional ``blockchain`` field which is the specific chain for which to limit the address.
