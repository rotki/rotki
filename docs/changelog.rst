...
* :bug:`12079` Switching the log level from the onboarding or settings screen now takes effect immediately across the frontend logger and Electron log file, and the onboarding dropdown no longer latches a stale default after a reset on production builds.
