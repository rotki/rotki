...
* :feature:`12889` Everything that needs your attention now lives in one place: a new action center next to the notification bell. It lists, by area, history events waiting for review, missing API keys and indexers, expired Gnosis Pay and Monerium sessions, exchange assets without a mapping, assets missing a price and how long ago history was last synced, and each row opens the place to fix it from any page. Its badge counts only what is new since you last opened it, and turns into a dot while older items are still waiting. "Remind me later" puts an item aside for a week, or until it grows, and "Remind me now" brings it back. A row goes away once its cause is fixed, so these no longer pile up as notifications, and the bell now only shows a dot for new messages. The history sync bar moves from the dashboard into the action center, and a failed asset search is now shown where you typed instead of as a notification.
...
* :feature:`10972` Bitcoin transactions are now also queried from a custom mempool API endpoint, so users running their own mempool instance get their transaction history without sharing their addresses with a third-party explorer.
* :feature:`13164` Aztec staking activity is now shown clearly in your history, including provider stakes, governance registration and delegation changes, and staking rewards. Staked AZTEC and undistributed rewards are also included in your portfolio balances.
* :feature:`-` The dashboard tables are easier to scan. Titles show how many rows each table holds, column headers are smaller and quieter, and a table short enough to fit on one page at any rows-per-page setting shows no pagination. A table that fits on one page only because rows per page was raised keeps its pagination, so the setting can be lowered again. The search field appears once a table has more than ten rows. NFTs and liquidity pools can now be refreshed from the "Refresh one source" menu next to the total, which replaces their separate refresh buttons. LP tokens show their pair above the pool type, and an asset whose name only repeats its symbol, such as ETH, shows it once.
* :feature:`-` The dashboard now shows where your net worth is held. Next to the total, a bar and legend split it between blockchain accounts, exchanges, banks, manual balances and NFTs, with liabilities underneath, and a strip of location tiles replaces the per-category summary cards. Picking a source in the legend narrows the tiles to it, and a place you hold through several sources, such as an exchange with manual balances recorded against it, shows as one tile. On a new account, an "Add a source" button there leads to adding an address, an exchange, a bank or a manual balance. A single Refresh button next to the total now queries every source and then prices, and its menu can redetect tokens first, refresh only prices, or refresh one source.
...
* :feature:`13121` 0x router is now supported in Robinhood chain.
...
* :feature:`9106` Ink Chain is now a supported EVM chain. Balances and transactions can be tracked on it. Transaction history on it needs a Blockscout API key.
...
* :bug:`-` Privacy mode no longer renders every balance as zero when the scramble multiplier is set to 0, which read as rotki losing your balances rather than concealing them. A multiplier between 0 and 1 is used as given and scrambles amounts downwards, and randomly generated multipliers now always start at 1.
...
* :feature:`13152` A matched pair of bridge events can now be unmatched from the history events list, the same way a matched deposit or withdrawal already could. Both legs are restored and then ignored, so automatic matching does not link them straight back, and they can be restored or matched by hand from the ignored tab of the bridge matching window. A counterpart event that was created to complete the match is removed with it.
* :bug:`-` Unmatching a pair of events is now offered on a matched event whose accounting rule is missing, where the button was previously replaced by the warning icon and the action could only be reached by expanding the pair. The preview of the events an accounting rule matches no longer offers to unmatch them, which it carried out for real even though nothing else in that dialog changes your data.
* :bug:`-` Unmatching an asset movement now ignores it as well, so automatic matching no longer links it straight back. It can be restored or matched by hand from the ignored tab of the asset movement matching window, where a row that is ignored now offers to find a match instead of only to be restored. The same applies to the ignored tab for bridge transactions.
...
* :feature:`-` Filtering accounting rules now uses the same filter bar as the rest of rotki: each filter is a pill, event types and subtypes are picked from a list and read as words rather than as their raw values, and counterparties are shown with their protocol icon and name.
* :feature:`-` Filtering internal transaction conflicts now uses the same filter bar as the rest of rotki, chains are shown with their logo, and the separate start and end date filters are now one date filter with a picker for each end.
* :feature:`-` Filtering custom assets now uses the same filter bar as the rest of the asset manager.
* :feature:`-` Filtering oracle prices now uses the same filter bar as the rest of rotki, and its two separate start and end date filters are now one period filter with a date picker for each end. Assets are shown with their icon and symbol, and each oracle by the name the table itself uses, instead of the raw identifier.
* :bug:`-` Changing the date filter on the Kraken staking page while an earlier query is still running no longer risks the table settling on the results of the filter you moved away from.
* :feature:`-` The date filter on the Kraken staking page now uses the same filter bar as the rest of rotki, and its two separate start and end date filters are now one date filter with a picker for each end.
...
* :bug:`-` Binance CSV imports now correctly recognize legacy NEO and ONT rewards and airdrops, asset recovery balance adjustments, and token swap distributions.
* :bug:`-` The chain picker for redecoding history events now offers every chain whose transactions can be redecoded, including evm-like chains and solana, rather than only the EVM ones. Those chains were already covered by Redecode All, but there was no way to redecode just one of them on its own. The button also reads "Redecode 1 chain" rather than "Recode 1 chains".
* :bug:`-` A misbehaving RPC node that replies with an error no longer kills whatever was querying it.
* :bug:`-` Activating or deactivating an RPC node from the settings no longer fails with "Unknown field".
* :feature:`-` Background operations such as balance queries and transaction decoding are now noticed as soon as they finish, instead of up to four seconds later. The difference adds up wherever many of them run one after another: the balance refresh after you log in used to pad every single step with that wait.
* :bug:`-` An address whose transaction query fails is now shown as failed in the sync progress panel instead of claiming to still be querying for the rest of the session. A chain whose addresses all failed no longer disappears from the panel, so the panel no longer reports every chain complete while some of them failed.
...
* :bug:`10625` Incomplete exchange history queries no longer advance their saved progress and lose events. Completed Binance trade pairs are retained when a later pair is rate limited.
...
* :feature:`5055` You can now run the rotki container on a port other than 80 by setting the ``ROTKI_HTTP_PORT`` environment variable. This unblocks setups where rotki shares a network namespace with another container that already uses port 80, such as behind a VPN container, where remapping only the published port is not enough. The container's health check follows the same port automatically.
* :feature:`-` Docker access logs are now in the standard combined format, so you can point tools such as goaccess or awstats straight at them, and they record the real visitor IP rather than your reverse proxy's own address. Forwarded IPs are trusted only from private or loopback peers; if your proxy reaches rotki from a public address, list it with ``--trusted-proxy``. rotki's own health check no longer clutters the log.
* :bug:`-` The rotki and colibri backends inside the Docker container now listen only within the container, so only the rotki port you publish is reachable and the internal APIs are no longer exposed on the network. With the default bridge networking nothing changes for you; only if you run with ``--network host`` and connected straight to ports 4242 or 4343 do you now need to go through the published port instead.
* :bug:`-` A mistake in ``/config/rotki_config.json`` now stops the Docker container with a clear error that names the problem, instead of being silently ignored. Previously the error was only logged and rotki carried on with default settings, so a typo could leave you running a configuration you believed you had applied.
...
* :feature:`5176` The warning shown when rotki runs in Docker now says what is actually wrong and how to fix it: that anyone who can reach the instance can use the API and read your data, and what to set to stop that. The ``ROTKI_ACCEPT_DOCKER_RISK`` environment variable is retired and no longer read. If you had set it, you will see the new warning once. Set ``ROTKI_SESSION_KEY`` to require authentication, which is the recommended fix, or ``ROTKI_ACCEPT_UNAUTHENTICATED_API`` to keep the instance open and silence the warning as before.
...
* :feature:`12756` Binance API keys now default history syncing to immediately after the latest imported Binance CSV entry, or to when the key is added if no CSV was imported. The start date can be adjusted before saving the key, avoiding impractical full-history API queries and their strict rate limits.
* :feature:`12094` You can now have rotki automatically scan for new tokens each time you log in (opt-in, disabled by default), and tune how often that on-login scan should actually run via a configurable cooldown between 1 hour and 7 days.
