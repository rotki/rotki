...
* :bug:`-` rotki will no longer auto-detect accounts on Optimism, Binance Smart Chain, and Base chains by default, as Etherscan ended free API access for these networks. Users with paid API keys can enable these chains by removing them from the chains to skip detection setting.
