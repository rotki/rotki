...
* :feature:`-` Linea is now a supported EVM chain. Balances and transactions can be tracked on it. Transaction history is queried via Etherscan (lineascan), so an Etherscan API key is needed.
