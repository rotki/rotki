...
* :feature:`13152` A matched pair of bridge events can now be unmatched from the history events list, the same way a matched deposit or withdrawal already could. Both legs go back to being unmatched bridge transactions, and a counterpart event that was created to complete the match is removed with it.
