...
* :bug:`-` Optimism, Base and Gnosis now say up front that they need a Blockscout key (the free plan is enough) or a paid Etherscan key. Adding an account on them without either used to show no warning, and when their queries then failed the notification only offered a paid Etherscan key.
