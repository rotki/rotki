...
* :feature:`-` The "Actions needed" button in history events has been reworked into a single actions center. It now counts the categories that need attention rather than every individual item, always opens the same panel, and lists each category as a row with its own count and action. Categories that are handled automatically, such as internal transaction conflicts, are shown as such, categories with nothing to do stay listed so you can re-check them at any time, and unmatched movements or bridge transactions you cannot act on without premium are shown greyed with the tier that unlocks them. The duplicated "Check ..." entries in the overflow menu are gone, since everything they did is now in the panel.
