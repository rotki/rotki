...
class Inquirer:
    ...
    def _try_oracle_price_query(
        ...
        if pos_matic_token is not None and pol_token in prices:
            prices[pos_matic_token] = prices[pol_token]
    ...
    def _find_prices(
        ...
        for from_asset in from_assets:
            ...
            if (collection_main_asset := GlobalDBHandler.get_collection_main_asset(from_asset.identifier)) is not None:  # noqa: E501
                unknown_prices.append(Asset(collection_main_asset).resolve_to_asset_with_oracles())
            else:
                unknown_prices.append(from_asset)

        if to_asset == A_USD:
            found_prices.update(Inquirer.find_usd_prices_and_oracles(
                assets=unknown_prices,
                ignore_cache=ignore_cache,
            ))
            return found_prices

        if ignore_cache is False:
            for from_asset in unknown_prices.copy():
                if (cache := Inquirer.get_cached_current_price_entry(cache_key=(from_asset, to_asset))) is not None:  # noqa: E501
                    ...
                    unknown_prices.remove(from_asset)

        # check manual prices
        prices, unknown_prices = Inquirer._try_oracle_price_query(  # type: ignore[assignment]  # Manual current oracle uses Asset type
            oracle=CurrentPriceOracle.MANUALCURRENT,
            oracle_instance=Inquirer._manualcurrent,
            from_assets=unknown_prices,  # type: ignore[arg-type]  # Manual current oracle can works with Asset type
    ...
    def find_price_and_oracle(
        ...
        """Wrapper around _find_prices to include
        the oracle queried when getting a single price.
        """
    ...
    def _find_usd_prices(
        ...
        for asset in assets:
            ...
            try:
                asset = asset.resolve()  # noqa: PLW2901  # we do want to overwrite the loop variable  # noqa: E501
            except UnknownAsset:
            ...
            assets_without_price.append(asset)
