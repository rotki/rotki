...
def configure_logging(args: argparse.Namespace) -> None:
    ...
    if not args.logfromothermodules:
        ...
        # TODO VN: remove
        logging.getLogger('rotkehlchen.chain.ethereum.manager').setLevel(logging.CRITICAL)
        logging.getLogger('web3.RequestManager').setLevel(logging.CRITICAL)
        logging.getLogger('web3.providers.HTTPProvider').setLevel(logging.CRITICAL)
        logging.getLogger('rotkehlchen.data_handler').setLevel(logging.CRITICAL)
        logging.getLogger('rotkehlchen.premium.sync').setLevel(logging.CRITICAL)
        logging.getLogger('rotkehlchen.icons').setLevel(logging.CRITICAL)
        logging.getLogger('rotkehlchen.utils.misc').setLevel(logging.CRITICAL)
        logging.getLogger('rotkehlchen.externalapis.etherscan').setLevel(logging.CRITICAL)
        logging.getLogger('rotkehlchen.externalapis.cryptocompare').setLevel(logging.CRITICAL)
        logging.getLogger('rotkehlchen.externalapis.coingecko').setLevel(logging.CRITICAL)
        logging.getLogger('rotkehlchen.chain.ethereum.tokens').setLevel(logging.CRITICAL)
        logging.getLogger('rotkehlchen.externalapis.cryptocompare').setLevel(logging.CRITICAL)
        logging.getLogger('rotkehlchen.exchanges.manager').setLevel(logging.CRITICAL)
        logging.getLogger('rotkehlchen.exchanges').setLevel(logging.CRITICAL)
        logging.getLogger('rotkehlchen.db.dbhandler').setLevel(logging.CRITICAL)
        logging.getLogger('rotkehlchen.chain.substrate.manager').setLevel(logging.CRITICAL)
        # logging.getLogger('rotkehlchen.inquirer').setLevel(logging.CRITICAL)
