...
class Cryptocom(ExchangeInterface, SignatureGeneratorMixin):
    ...
    def _deserialize_deposit(self, raw_result: dict[str, Any]) -> list[AssetMovement]:
        return self._deserialize_asset_movement(
            raw_result=raw_result,
            event_type=HistoryEventType.DEPOSIT,
        )

    def _deserialize_withdrawal(self, raw_result: dict[str, Any]) -> list[AssetMovement]:
        return self._deserialize_asset_movement(
            raw_result=raw_result,
    ...
    def _deserialize_asset_movement(
        ...
        return create_asset_movement_with_fee(
            ...
            fee=AssetAmount(
                asset=asset,
                amount=deserialize_fval_or_zero(raw_result.get('fee', '0')),
            ),
    ...
    def _deserialize_trade(self, raw_result: dict[str, Any]) -> list[SwapEvent]:
        ...
        fee = deserialize_fval_force_positive(raw_result.get('fees', '0'))
        ...
        return create_swap_events(
            ...
            spend=spend,
            receive=receive,
            fee=AssetAmount(asset=base_asset, amount=fee) if fee > ZERO else None,
            location_label=self.name,
    ...
    def _query_trades(
        ...
        if (now := ts_now_in_ms()) - end_ts > MAX_TRADES_AGE:
            log.error(f'Cannot query {self.name} trades older than 6 months.')
            return []
        ...
        while True:
            events.extend(self._query_paginated(
                start_ts=TimestampMS(from_ts),
                end_ts=TimestampMS(to_ts),
                query_type='trade',
            ))
            if to_ts >= end_ts:
                break
    ...
    def _query_paginated(
        ...
        if query_type == 'trade':
            options = {
                'start_time': ts_ms_to_ns(start_ts),
                'limit': (page_size := TRADES_LIMIT),
            }
            method = 'private/get-trades'
            result_key = 'data'
            page_key = 'end_time'
            initial_page = ts_ms_to_ns(end_ts)
            deserialize_fn = self._deserialize_trade
        else:
            options = {
                'start_ts': start_ts,
                'end_ts': end_ts,
                'page_size': (page_size := DEPOSIT_WITHDRAWAL_PAGE_SIZE),
            }
            page_key = 'page'
            initial_page = 0
            if query_type == 'deposit':
                method = 'private/get-deposit-history'
                options['status'] = DEPOSIT_ARRIVED_STATUS
                result_key = 'deposit_list'
                deserialize_fn = self._deserialize_deposit
            else:  # withdraw
                method = 'private/get-withdrawal-history'
                options['status'] = WITHDRAWAL_COMPLETED_STATUS
                result_key = 'withdrawal_list'
                deserialize_fn = self._deserialize_withdrawal

        options[page_key] = initial_page
        events: list[HistoryBaseEntry] = []
        while True:
            try:
                result = self._process_response(self._api_query(
                    method=method,  # type: ignore[arg-type]  # set to a valid value above
                    options=options,
                ))
    ...
    def query_online_history_events(
        ...
        self.first_connection()
        events: list[HistoryBaseEntry] = []
        start_ts_ms = ts_sec_to_ms(start_ts)
        end_ts_ms = ts_sec_to_ms(end_ts)
        for query_type in ('deposit', 'withdrawal'):
            events.extend(self._query_paginated(
