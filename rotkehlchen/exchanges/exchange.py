...
class ExchangeInterface(CacheableMixIn, LockableQueryMixIn):
    ...
    def query_online_history_events(
        ...
        """
        # Should raise NotImplementedError here after all exchanges are updated
        return []
    ...
    def query_other_history_events(self) -> None:
        ...
        An alternative to query_history_events and query_online_history_events used in
        special cases when the logic in query_history_events is not applicable, for instance
        to handle history events from multiple portfolios in coinbaseprime.
        """
        return None
    ...
    def query_history_events(
        ...
        """Queries the local DB and the exchange for history events.
        ...
        with self.db.conn.read_ctx() as cursor:
            return db.get_history_events(cursor, filter_query=filter_query, has_premium=True)
