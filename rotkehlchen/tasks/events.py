...
def _should_auto_ignore_movement(asset_movement: AssetMovement) -> bool:
    ...
    """
    if asset_movement.location == LOCATION_COINBASEPRO:
        return True

    if (
        asset_movement.location == LOCATION_COINBASE and
...
def update_asset_movement_matched_event(
    ...
    if isinstance(matched_event, OnchainEvent):
        ...
        matched_event.counterparty = asset_movement.location
    ...
    if should_edit_notes:
        format_args = {
            ...
            'exchange': asset_movement.location.capitalize(),
        }
    ...
    matched_event.extra_data[MATCHED_ASSET_MOVEMENT_KEY] = {
        'group_identifier': asset_movement.group_identifier,
        'exchange': asset_movement.location,
    }
    ...
    with events_db.db.conn.write_ctx() as write_cursor:
        ...
        if isinstance(matched_event, AssetMovement):
            ...
            asset_movement.extra_data[MATCHED_ASSET_MOVEMENT_KEY] = {
                ...
                'exchange': matched_event.location,
            }
            events_db.edit_history_event(
                write_cursor=write_cursor,
...
def should_exclude_possible_match(
    ...
    if (
        ...
        event.location_label in (chain_accounts := blockchain_accounts.get(
            location_to_chain(event.location),
        )) and
        ((  # btc/bch don't have the address attribute so can only go by a tracked location label.
            ...
            event.location in (LOCATION_BITCOIN, LOCATION_BITCOIN_CASH)
