...
class AsyncBalanceUpdater:
    ...
    async def update_balances(self, addresses: list[str] | None = None):
        ...
        try:
            ...
            tasks = []
            for chain in self.chains_aggregator.get_chains():
                if hasattr(chain, 'async_node_inquirer'):
                    tasks.append(
                        self._update_chain_balances(chain, addresses)
                    )
