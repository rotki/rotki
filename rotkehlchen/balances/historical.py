...
class HistoricalBalancesManager:
    ...
    def get_balances(self, timestamp: Timestamp) -> dict[Asset, HistoricalBalance]:
        ...
        for event in events:
            if self._update_balances(event, current_balances) is not None:
                break
    ...
    def get_assets_amounts(
        ...
        negative_balance_event = None
        current_balances: dict[Asset, FVal] = defaultdict(FVal)
        amounts: dict[Timestamp, FVal] = defaultdict(lambda: ZERO)
        for event in events:
            if (event_id := self._update_balances(event, current_balances)) is not None:
                negative_balance_event = event_id
                break
        ...
        return amounts, negative_balance_event

    def get_historical_netvalue(
            self,
            from_ts: Timestamp,
            to_ts: Timestamp,
    ) -> tuple[dict[Timestamp, FVal], list[tuple[str, Timestamp]], str | None]:
        ...
        Returns:
            ...
            - The event_identifier that caused a negative balance if any, else None
        """
        ...
        for event in events:
            ...
            if (negative_id := self._update_balances(event, current_balances)) is not None:
                negative_balance_event = negative_id
                break

        if negative_balance_event is None:
            daily_balances[current_day] = current_balances.copy()
        ...
        return net_worth_per_day, missing_price_points, negative_balance_event
    ...
    def _get_events_and_currency(
        ...
        with self.db.conn.read_ctx() as cursor1, self.db.conn.read_ctx() as cursor2:
            ...
            event_filters: list[DBFilter] = [DBTimestampFilter(
                and_op=True,
                to_ts=to_ts,
                from_ts=from_ts,
            ...
            # Optional assets param allows us fetch all events or filter for specific assets.
            # For trades, we check both base and quote assets since trades affect both.
            # For history events we only check the single affected asset.
            ...
            while trades_buffer or events_buffer:
                try:
                    ...
                    if process_event:
                        ...
                        event = HistoryEvent.deserialize_from_db(events_buffer.popleft()[1:])
                        events.append(event)
                        if len(events_buffer) == 0:  # refill trades buffer if empty
                            events_buffer.extend(cursor2.fetchmany(BATCH_SIZE))
                    else:
                        trade = Trade.deserialize_from_db(trades_buffer.popleft())
                        events.append(trade)
    ...
    def _update_balances(
        ...
        Returns the event_identifier if a negative balance would occur, else None.
        """
        if isinstance(event, Trade):
