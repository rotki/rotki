...
from rotkehlchen.types import CacheType, ChainID, UniqueCacheType
...
def test_yearn_api_addresses_are_checksummed(
        database: DBHandler,
    ...
    def mock_yearn_api(url: str, timeout: int) -> MockResponse:  # pylint: disable=unused-argument
...
@pytest.mark.parametrize('run_globaldb_migrations', [False])
@pytest.mark.parametrize('custom_globaldb', ['v4_global_before_migration1.db'])
def test_yearn_api(database: DBHandler, ethereum_inquirer: EthereumInquirer) -> None:
    ...
    yearn_vaults_cache_key: tuple[UniqueCacheType, str] = (
        CacheType.YEARN_VAULTS, str(ChainID.ETHEREUM.serialize_for_db()),
    )

    def mock_yearn_api(url: str, timeout: int) -> MockResponse | requests.Response:
        ...
        return original_request(url, timeout=timeout)
