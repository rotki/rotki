...
def test_unmatched_bridge_data_issues(
    ...
    assert {
        (issue.payload['direction'], LocationIdentifier(issue.location), issue.state)
        for issue in issues
    } == {
        ('deposit', LOCATION_ETHEREUM, IssueState.OPEN.value),
        ('withdrawal', LOCATION_ARBITRUM_ONE, IssueState.OPEN.value),
    }
