...
def test_locked_velo_calendar_reminders(
    ...
    assert calendar_entry == CalendarEntry(
        ...
        description=f'Lock period for VELO veNFT-30123 in vote escrow ends on {reminder_creator.timestamp_to_date(events[0].extra_data["lock_time"])}',  # noqa: E501
        counterparty=CPT_VELODROME,
        blockchain=ChainID.deserialize(events[0].location.to_chain_id()).to_blockchain(),
