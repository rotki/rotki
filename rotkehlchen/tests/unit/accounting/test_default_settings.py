...
def _gain_one_ether(
    ...
    consumed_num = transactions.process(
...
def test_accounting_no_settings(accounting_pot: 'AccountingPot'):
    ...
    consumed_num = accounting_pot.transactions.process(event=event, events_iterator=iter([]))
...
def test_accounting_receive_settings(
    ...
    if entry_type == 'history_event':
        expected_tx_hash = f'rotki_events_{EXAMPLE_TX_HASH_HEX}'
    else:  # entry_type == 'evm_event'
        expected_tx_hash = EXAMPLE_TX_HASH_HEX
    ...
    expected_event = ProcessedAccountingEvent(
        ...
        cost_basis=None,
        index=0,
        extra_data={'tx_hash': expected_tx_hash},
    )
...
def test_accounting_spend_settings(
    ...
    consumed_num = accounting_pot.transactions.process(event=spend_event, events_iterator=iter([]))
    assert consumed_num == 1
...
def test_accounting_swap_settings(accounting_pot: 'AccountingPot', counterparty: str):
    ...
    expected_spend_event = ProcessedAccountingEvent(
        ...
        index=1,
        extra_data={'tx_hash': EXAMPLE_TX_HASH_HEX, 'group_id': EXAMPLE_TX_HASH_HEX + '12'},
    )
