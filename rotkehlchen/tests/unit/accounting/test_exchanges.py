...
def test_account_for_coinbase_income_expense(rotkehlchen_api_server_with_exchanges: APIServer) -> None:  # noqa: E501
    """
    ...
    with patch.object(coinbase.session, 'get', side_effect=mock_normal_coinbase_query):
        report, events = accounting_create_and_process_history(rotki=rotki, start_ts=Timestamp(0), end_ts=Timestamp(1611426233))  # noqa: E501

    expected_total_actions = 10
    assert report['total_actions'] == expected_total_actions
    events_map: defaultdict[AccountingEventType, int] = defaultdict(int)
    for event in events:
...
@pytest.mark.parametrize('initialize_accounting_rules', [True])
def test_exchanges_removed_api_keys(rotkehlchen_api_server_with_exchanges: APIServer) -> None:
    """
...
@pytest.mark.parametrize('ethereum_accounts', [[]])
@pytest.mark.parametrize('added_exchanges', [[]])
def test_process_kraken_events(rotkehlchen_api_server_with_exchanges: APIServer) -> None:
    """
