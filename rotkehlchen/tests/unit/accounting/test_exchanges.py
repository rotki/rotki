...
def test_account_for_coinbase_income_expense(rotkehlchen_api_server_with_exchanges):
    ...
    coinbase = rotki.exchange_manager.get_exchange_by_name(LOCATION_COINBASE, 'coinbase')
...
def test_exchanges_removed_api_keys(rotkehlchen_api_server_with_exchanges: APIServer):
    ...
    rotki.exchange_manager.delete_exchange(rotki.exchange_manager.connected_exchanges[LOCATION_COINBASE][0].connection_identifier)
    ...
    assert event1.location == LOCATION_COINBASE
    ...
    assert event2.location == LOCATION_COINBASE
