...
def test_match_asset_movements(database: 'DBHandler') -> None:
    ...
    assert deposit2_matched_event.notes == 'Withdraw 0.1 ETH to Gemini 2'
    ...
    assert withdrawal1_matched_event.notes == 'Deposit 0.2 USDC from Coinbase 1'
    ...
    # The remaining two events are related to withdrawal2,
    # but neither are modified since both match.
    assert all(event.notes is None for event in events[5:])
    ...
    with database.conn.read_ctx() as cursor:
        assert cursor.execute(
            ...
            (f'matched_asset_movement_{withdrawal1_matched_event.identifier}', str(withdrawal1_identifier := 5)),  # noqa: E501
        ]
        matched_asset_movements = events_db.get_history_events_internal(
            cursor=cursor,
            filter_query=HistoryEventFilterQuery.make(
                identifiers=[deposit_2_identifier, withdrawal1_identifier],
            ),
        )
    assert len(matched_asset_movements) == 2
    deposit2.identifier = deposit_2_identifier
    assert matched_asset_movements[0] == deposit2
    withdrawal1.identifier = withdrawal1_identifier
