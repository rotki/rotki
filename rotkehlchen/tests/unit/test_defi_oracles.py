...
def test_uniswap_oracles_evm(inquirer_defi_evm: 'Inquirer') -> None:
    """Test that Uniswap V2 and V3 oracles return correct prices in evm chains"""
    assert inquirer_defi_evm._uniswapv2 is not None
    assert inquirer_defi_evm._uniswapv3 is not None

    # v2 historical on base
    assert inquirer_defi_evm._uniswapv2.query_historical_price(
        from_asset=A_ETH,
    ...
    assert inquirer_defi_evm._uniswapv2.query_current_price(
        from_asset=A_ETH,
        to_asset=A_ARBITRUM_USDC,
    ) == Price(FVal('3212.0656284514484'))

    # v3 historical on polygon
    assert inquirer_defi_evm._uniswapv3.query_historical_price(
    ...
    assert inquirer_defi_evm._uniswapv3.query_current_price(
        from_asset=A_ETH,
