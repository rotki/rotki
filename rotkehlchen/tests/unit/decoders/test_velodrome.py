...
def test_increase_unlock_time(optimism_accounts, optimism_transaction_decoder):
    ...
    expected_events = [
        EvmEvent(
            ...
            counterparty=CPT_VELODROME,
            notes='Increase unlock time to 12/06/2025',
        ),
    ]
    assert events == expected_events
...
def test_claim_bribes(optimism_accounts, optimism_transaction_decoder):
    ...
    expected_events = [
        EvmEvent(
            ...
            counterparty=CPT_VELODROME,
            notes=f'Claim {receive_amount_1} T from velodrome voter contract',
        ), EvmEvent(
            tx_hash=evmhash,
            sequence_index=145,
            timestamp=timestamp,
            ...
            address=string_to_evm_address('0x9015Af1A94d0c9896DB9BF359b62dc9B114d5587'),
            counterparty=CPT_VELODROME,
            notes=f'Claim {receive_amount_2} OP from velodrome voter contract',
        ), EvmEvent(
            tx_hash=evmhash,
            sequence_index=147,
            timestamp=timestamp,
            ...
            address=string_to_evm_address('0x9015Af1A94d0c9896DB9BF359b62dc9B114d5587'),
            counterparty=CPT_VELODROME,
            notes=f'Claim {receive_amount_3} ITP from velodrome voter contract',
        ), EvmEvent(
            tx_hash=evmhash,
            sequence_index=149,
            timestamp=timestamp,
            ...
            counterparty=CPT_VELODROME,
            notes=f'Claim {receive_amount_4} tBTC from velodrome voter contract',
        ),
    ]
    assert events == expected_events
