...
def _add_velodrome_pool(pool: ChecksumEvmAddress) -> None:
    """Add a velodrome pool to the cache so decoding is properly triggered for related events."""
...
def test_add_liquidity_v2(
    ...
    assert (
        GlobalDBHandler.get_evm_token(address=WETH_OP_POOL_ADDRESS, chain_id=ChainID.OPTIMISM)
        is None
    )
    user_address = optimism_accounts[0]
    events, _ = get_decoded_events_of_transaction(
        evm_inquirer=cast('OptimismInquirer', optimism_transaction_decoder.evm_inquirer),
        tx_hash=(
            tx_hash := deserialize_evm_tx_hash(
                '0xa1cf038ca08b51971314b4f8e006b455f2a82f113899e5eb7818ac1528605bfc',
            )
        ),
    ...
    [
        {
            'evm_indexers_order': SerializableChainIndexerOrder(
                {ChainID.OPTIMISM: [EvmIndexer.ROUTESCAN]},
            ),
        },
    ],
)
...
def test_add_liquidity_v1(
    ...
    events, _ = get_decoded_events_of_transaction(
        ...
        tx_hash=(
            tx_hash := deserialize_evm_tx_hash(
                '0x779c2c762ed7ca5d2236ac548f9e9bdc723a8bc9d6d8feba9e3baea9456bbac9',
            )
        ),
        load_global_caches=load_global_caches,
    )
...
def test_remove_liquidity_v2(
    ...
    events, _ = get_decoded_events_of_transaction(
        ...
        tx_hash=(
            tx_hash := deserialize_evm_tx_hash(
                '0x81351bf9ca6d78bff92d2b714d68dd7c785bb70de0e250bc7bc6ab073736d1ce',
            )
        ),
        load_global_caches=load_global_caches,
    )
...
def test_remove_liquidity_v1(
    ...
    events, _ = get_decoded_events_of_transaction(
        ...
        tx_hash=(
            tx_hash := deserialize_evm_tx_hash(
                '0x9b564c266aea871c14fbe120d060d1713f0ba524452906af3592360594b946f6',
            )
        ),
        load_global_caches=load_global_caches,
    )
        ...
        EvmEvent(
            ...
            asset=Asset(
                evm_address_to_identifier(
                    address=pool, chain_id=ChainID.OPTIMISM, token_type=TokenKind.ERC20,
                ),
            ),
            amount=ZERO,
            location_label=user_address,
...
def test_swap_eth_to_token_v2(
    ...
    events, _ = get_decoded_events_of_transaction(
        ...
        tx_hash=(
            tx_hash := deserialize_evm_tx_hash(
                '0x8d65dc5a77aaceabac3b80fd28f8fa3cd45143748c2d31598706580f68d724da',
            )
        ),
        load_global_caches=load_global_caches,
    )
...
def test_swap_eth_to_token_v1(
    ...
    events, _ = get_decoded_events_of_transaction(
        ...
        tx_hash=(
            tx_hash := deserialize_evm_tx_hash(
                '0xaa6b816c09863eab80f6184ab4112d7fea3a813c194dcebf08b6d4e13df22354',
            )
        ),
        load_global_caches=load_global_caches,
    )
...
def test_swap_token_to_eth_v2(
    ...
    events, _ = get_decoded_events_of_transaction(
        ...
        tx_hash=(
            tx_hash := deserialize_evm_tx_hash(
                '0x15c46045b9a4b03d15f0260f6518563f9a050fd693712330d9c05bedef833886',
            )
        ),
        load_global_caches=load_global_caches,
    )
...
def test_swap_token_to_eth_v1(
    ...
    events, _ = get_decoded_events_of_transaction(
        ...
        tx_hash=(
            tx_hash := deserialize_evm_tx_hash(
                '0x10d52ec5cf06d1b5b48a346334c8f0e6ef83bd281c371d762d3c163e37f629d8',
            )
        ),
        load_global_caches=load_global_caches,
    )
    assert events == [
        ...
        EvmSwapEvent(
            ...
            counterparty=CPT_VELODROME,
            address=(
                address := string_to_evm_address('0xe8537b6FF1039CB9eD0B71713f697DDbaDBb717d')
            ),  # velo_usdc_pool_address
...
def test_swap_tokens_v2(
    ...
    events, _ = get_decoded_events_of_transaction(
        ...
        tx_hash=(
            tx_hash := deserialize_evm_tx_hash(
                '0x8c36e51bc4d3deec6a4c06a59179083adfd9caa8a5ad7e957e639d5792c9e139',
            )
        ),
        load_global_caches=load_global_caches,
    )
...
def test_swap_tokens_v1(
    ...
    events, _ = get_decoded_events_of_transaction(
        ...
        tx_hash=(
            tx_hash := deserialize_evm_tx_hash(
                '0x398538615367f778e9bb7fd95b3f96ee52e2192880b6f6331f91530ae4c829d9',
            )
        ),
        load_global_caches=load_global_caches,
    )
...
@pytest.mark.parametrize('load_global_caches', [[CPT_VELODROME]])
@pytest.mark.parametrize('optimism_accounts', [['0x78C13393Aee675DD7ED07ce992210750D1F5dB88']])
def test_stake_lp_token_to_gauge_v2(
        optimism_accounts: list[ChecksumEvmAddress],
        optimism_transaction_decoder: OptimismTransactionDecoder,
        load_global_caches: list[str],
) -> None:
    ...
    events, _ = get_decoded_events_of_transaction(
        ...
        tx_hash=(
            tx_hash := deserialize_evm_tx_hash(
                '0x1bfa588dc839b13205e80bbfd7b7748a4c599854a03b52bb5476d6edae2e95a9',
            )
        ),
        load_global_caches=load_global_caches,
    )
...
@pytest.mark.parametrize('load_global_caches', [[CPT_VELODROME]])
@pytest.mark.parametrize('optimism_accounts', [['0x78C13393Aee675DD7ED07ce992210750D1F5dB88']])
def test_unstake_lp_token_to_gauge_v2(
        optimism_accounts: list[ChecksumEvmAddress],
        optimism_transaction_decoder: OptimismTransactionDecoder,
        load_global_caches: list[str],
) -> None:
    ...
    events, _ = get_decoded_events_of_transaction(
        ...
        tx_hash=(
            tx_hash := deserialize_evm_tx_hash(
                '0xe774e27053cab2c9cc293d6b1010d024a9a6af57fb6645ca52f0c5a26d117eeb',
            )
        ),
        load_global_caches=load_global_caches,
    )
...
def test_get_reward_from_gauge_v2(
    ...
    events, _ = get_decoded_events_of_transaction(
        ...
        tx_hash=(
            tx_hash := deserialize_evm_tx_hash(
                '0x9d0eae3c2d2cda853e77a2571a7c702507beb015c08b40406e03baa4b1dde505',
            )
        ),
        load_global_caches=load_global_caches,
    )
...
def test_unlock_velo(
    ...
    user_address, tx_hash = (
        optimism_accounts[0],
        deserialize_evm_tx_hash(
            '0x4389501a597f87f6f4c9042704f0040e5327251857d9a5043e4efff873787862',
        ),
    )
    events, _ = get_decoded_events_of_transaction(
        evm_inquirer=cast('OptimismInquirer', optimism_transaction_decoder.evm_inquirer),
...
def test_lock_velo(
    ...
    user_address, tx_hash = (
        optimism_accounts[0],
        deserialize_evm_tx_hash(
            '0xd5e3d9c5142cf4dd948ad5582a9fa3392e21238a2ae698cb73bafd8c4e02101f',
        ),
    )
    events, _ = get_decoded_events_of_transaction(
        evm_inquirer=cast('OptimismInquirer', optimism_transaction_decoder.evm_inquirer),
...
def test_increase_locked_amount(
    ...
    user_address, tx_hash = (
        optimism_accounts[0],
        deserialize_evm_tx_hash(
            '0x56a1e78a374981e5ebcfb605cd0835bee4b73a3221b908789fc917d11619ac9b',
        ),
    )
    events, _ = get_decoded_events_of_transaction(
        evm_inquirer=cast('OptimismInquirer', optimism_transaction_decoder.evm_inquirer),
...
def test_increase_unlock_time(
    ...
    user_address, tx_hash = (
        optimism_accounts[0],
        deserialize_evm_tx_hash(
            '0x513039d46a9b541e2cb7feb798060271a7b58e9e9ed80681c5d5b18f26fb8bfc',
        ),
    )
    events, _ = get_decoded_events_of_transaction(
        evm_inquirer=cast('OptimismInquirer', optimism_transaction_decoder.evm_inquirer),
...
def test_claim_bribes(
    ...
    user_address, tx_hash = (
        optimism_accounts[0],
        deserialize_evm_tx_hash(
            '0xfc2df8e001236e5d4d2026f7d9943ef782d225af8676189f684347b1053776bc',
        ),
    )
