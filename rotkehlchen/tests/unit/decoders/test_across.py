...
def test_across_bridge_deposit_on_ethereum(ethereum_inquirer, ethereum_accounts):
    ...
    assert events == [
        EvmEvent(
            ...
            asset=A_ETH,
            amount=FVal('0.000078206217311286'),
            location_label=user_address,
...
def test_across_add_liquidity_on_ethereum(ethereum_inquirer, ethereum_accounts):
    ...
    assert events == [
        EvmEvent(
            ...
            event_subtype=HistoryEventSubType.FEE,
            asset=A_ETH,
            amount=FVal('0.000078615407463768'),
            location_label=user_address,
            counterparty=CPT_GAS,
        ), EvmEvent(
            tx_ref=tx_hash,
...
def test_across_stake_lp_on_ethereum(ethereum_inquirer, ethereum_accounts):
    ...
    assert events == [
        EvmEvent(
            ...
            event_subtype=HistoryEventSubType.FEE,
            asset=A_ETH,
            amount=FVal('0.000149178245272874'),
            location_label=user_address,
            counterparty=CPT_GAS,
        ), EvmEvent(
            tx_ref=tx_hash,
...
def test_across_unstake_lp_on_ethereum(ethereum_inquirer, ethereum_accounts):
    ...
    assert events == [
        EvmEvent(
            ...
            event_subtype=HistoryEventSubType.FEE,
            asset=A_ETH,
            amount=FVal('0.000222559515065628'),
            location_label=user_address,
            counterparty=CPT_GAS,
        ), EvmEvent(
            tx_ref=tx_hash,
