...
def test_bulkcheckout_send_token(ethereum_inquirer, ethereum_accounts):
    ...
    assert events == [
        EvmEvent(
            ...
            event_subtype=HistoryEventSubType.FEE,
            asset=A_ETH,
            amount=FVal('0.000883451092980928'),
            location_label=user_address,
            counterparty=CPT_GAS,
        ), EvmEvent(
            tx_ref=tx_hash,
...
def test_gitcoin_vote_cast(ethereum_inquirer):
    ...
    assert events == [
        EvmEvent(
            ...
            event_subtype=HistoryEventSubType.FEE,
            asset=A_ETH,
            amount=FVal('0.001115205340736039'),
            location_label=ADDY,
            counterparty=CPT_GAS,
        ), EvmEvent(
            tx_ref=tx_hash,
...
def test_gitcoin_gr15_matching_claim(ethereum_inquirer, ethereum_accounts):
    ...
    assert events == [
        EvmEvent(
            ...
            event_subtype=HistoryEventSubType.FEE,
            asset=A_ETH,
            amount=FVal('0.00118573107792279'),
            location_label=user,
            counterparty=CPT_GAS,
        ), EvmEvent(
            tx_ref=tx_hash,
...
def test_gitcoin_payout_claimed_matching_gr12(ethereum_inquirer, ethereum_accounts):
    ...
    assert events == [
        EvmEvent(
            ...
            event_subtype=HistoryEventSubType.FEE,
            asset=A_ETH,
            amount=FVal('0.00327700462232052'),
            location_label=user,
            counterparty=CPT_GAS,
        ), EvmEvent(
            tx_ref=tx_hash,
...
def test_gitcoin_payout_claimed_matching_gr11(ethereum_inquirer, ethereum_accounts):
    ...
    assert events == [
        EvmEvent(
            ...
            event_subtype=HistoryEventSubType.FEE,
            asset=A_ETH,
            amount=FVal('0.00206346594437105'),
            location_label=user,
            counterparty=CPT_GAS,
        ), EvmEvent(
            tx_ref=tx_hash,
