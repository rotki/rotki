...
def test_aave_v3_withdraw_eth(scroll_inquirer, scroll_accounts) -> None:
    ...
    expected_events = [
        EvmEvent(
            ...
            location_label=scroll_accounts[0],
            notes=f'Receive {gained_amount} ETH as interest earned from AAVE v3',
            counterparty=CPT_AAVE_V3,
            address=ZERO_ADDRESS,
        ),
    ]
...
def test_aave_v3_withdraw_matic(polygon_pos_inquirer, polygon_pos_accounts) -> None:
    ...
    expected_events = [
        EvmEvent(
            ...
            location_label=polygon_pos_accounts[0],
            notes=f'Receive {gained_amount} POL as interest earned from AAVE v3',
            counterparty=CPT_AAVE_V3,
            address=ZERO_ADDRESS,
        ),
    ]
...
def test_aave_v3_withdraw_xdai(gnosis_inquirer, gnosis_accounts) -> None:
    ...
    expected_events = [
        EvmEvent(
            ...
            balance=Balance(FVal(gained_amount)),
            location_label=gnosis_accounts[0],
            notes=f'Receive {gained_amount} XDAI as interest earned from AAVE v3',
            counterparty=CPT_AAVE_V3,
            address=ZERO_ADDRESS,
        ),
    ]
...
def test_aave_v3_close_position_with_arbitrum_one(arbitrum_one_inquirer, arbitrum_one_accounts) -> None:    # noqa: E501
    tx_hash = deserialize_evm_tx_hash('0x56c57b83508f46d45af6b3230ee882f99df315c089af3250fbe5494ea59e5624')  # noqa: E501
    events, _ = get_decoded_events_of_transaction(
        evm_inquirer=arbitrum_one_inquirer,
        tx_hash=tx_hash,
    )
    timestamp = TimestampMS(1736028280000)
    # Note for the reviewer: I would prefer splitting this into multiple lines
    # but I was told in another PR to keep it in one line. Please let me know
    user_eoa_account, user_safe_proxy, eth_interest_amount, eth_withdraw_amount, usd_paid_back_amount, gas_fees = arbitrum_one_accounts[0], arbitrum_one_accounts[1], '0.031127570161941882', '3.007038496271076913', '2353.18136', '0.00000658667'  # noqa: E501
    ...
    expected_events = [
        EvmEvent(
            ...
            balance=Balance(amount=FVal(eth_withdraw_amount) + FVal(eth_interest_amount)),
            location_label=user_safe_proxy,
            # New bug, this is a WETH unwrapping operation, not a spend
