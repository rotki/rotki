...
def test_self_funded_deposit(ethereum_inquirer, ethereum_accounts):
    ...
    assert events == [
        EvmEvent(
            ...
            asset=A_ETH,
            amount=FVal('0.000013852978181768'),
...
def test_withdrawal_to_owner(ethereum_inquirer, ethereum_accounts):
    ...
    assert events == [
        EvmEvent(
            ...
            event_subtype=HistoryEventSubType.FEE,
            asset=A_ETH,
            amount=FVal('0.000015188280202674'),
            location_label=(user_address := ethereum_accounts[0]),
            counterparty=CPT_GAS,
        ), EvmEvent(
            tx_ref=tx_hash,
...
def test_withdrawal_to_another_address(ethereum_inquirer, ethereum_accounts):
    ...
    assert events == [
        EvmEvent(
            ...
            event_subtype=HistoryEventSubType.FEE,
            asset=A_ETH,
            amount=FVal('0.00016489225696348'),
            location_label=(user_address := ethereum_accounts[0]),
            counterparty=CPT_GAS,
        ), EvmEvent(
            tx_ref=tx_hash,
...
def test_interest_collection(ethereum_inquirer, ethereum_accounts):
    ...
    assert events == [
        EvmEvent(
            ...
            event_subtype=HistoryEventSubType.FEE,
            asset=A_ETH,
            amount=FVal('0.00001716401425758'),
            location_label=(user_address := ethereum_accounts[0]),
            counterparty=CPT_GAS,
        ), EvmEvent(
            tx_ref=tx_hash,
...
def test_deposit_base(base_inquirer, base_accounts):
    ...
    assert events == [
        EvmEvent(
            ...
            event_subtype=HistoryEventSubType.FEE,
            asset=A_ETH,
            amount=FVal('0.000000430021640992'),
...
def test_deposit_optimism(optimism_inquirer, optimism_accounts):
    ...
    assert events == [
        EvmEvent(
            ...
            event_subtype=HistoryEventSubType.FEE,
            asset=A_ETH,
            amount=FVal('0.000001133643733036'),
...
def test_deposit_polygon_pos(polygon_pos_inquirer, polygon_pos_accounts):
    ...
    assert events == [
        EvmEvent(
            ...
            event_subtype=HistoryEventSubType.FEE,
            asset=A_POL,
            amount=FVal('0.050978100548580012'),
...
def test_deposit_gnosis(gnosis_inquirer, gnosis_accounts):
    ...
    assert events == [
        EvmEvent(
            ...
            event_subtype=HistoryEventSubType.FEE,
            asset=A_XDAI,
            amount=FVal('0.0000871440130716'),
