...
def test_uniswap_v3_create_lp_position_with_native_refund(
    ...
    assert events == [EvmEvent(
        ...
        amount=FVal('1157920892373161954235709850086879078532699846656405640394575840079131.29623426'),
        location_label=user_address,
...
def test_swap_on_base(base_inquirer, base_accounts):
    ...
    assert events == [
        EvmEvent(
            ...
            event_subtype=HistoryEventSubType.FEE,
            asset=A_ETH,
            amount=FVal('0.000189731906791024'),
...
def test_swap_on_optimism(optimism_inquirer, optimism_accounts):
    ...
    assert events == [
        EvmEvent(
            ...
            event_subtype=HistoryEventSubType.FEE,
            asset=A_ETH,
            amount=FVal('0.000335793972468462'),
...
def test_swap_on_polygon_pos(polygon_pos_inquirer, polygon_pos_accounts):
    ...
    assert events == [
        EvmEvent(
            ...
            event_subtype=HistoryEventSubType.FEE,
            asset=A_POL,
            amount=FVal('0.025131590391178764'),
...
def test_add_liquidity_on_optimism(optimism_inquirer, optimism_accounts):
    ...
    assert events == [
        EvmEvent(
            ...
            event_subtype=HistoryEventSubType.FEE,
            asset=A_ETH,
            amount=FVal('0.000027353637451875'),
            location_label=optimism_accounts[0],
            counterparty=CPT_GAS,
        ), EvmEvent(
            tx_ref=tx_hash,
            ...
            event_subtype=HistoryEventSubType.APPROVE,
            asset=A_OP,
            amount=FVal('0.000129292741769402'),
            location_label=optimism_accounts[0],
            address=string_to_evm_address('0xC36442b4a4522E871399CD717aBDD847Ab11FE88'),
        ), EvmEvent(
            tx_ref=tx_hash,
...
def test_uniswap_v3_universal_router_2(ethereum_inquirer, ethereum_accounts):
    ...
    assert events == [
        EvmEvent(
            ...
            event_subtype=HistoryEventSubType.FEE,
            asset=A_ETH,
            amount=FVal('0.003983465486960712'),
