...
def test_gwei_names_commit(ethereum_inquirer, ethereum_accounts):
    ...
    assert events == [
        EvmEvent(
            ...
            asset=A_ETH,
            amount=FVal('0.00000791364626367'),
...
def test_gwei_names_register(ethereum_inquirer, ethereum_accounts):
    ...
    assert events == [
        EvmEvent(
            ...
            event_subtype=HistoryEventSubType.FEE,
            asset=A_ETH,
            amount=FVal('0.000022063022134344'),
            location_label=(user_address := ethereum_accounts[0]),
            counterparty=CPT_GAS,
        ), EvmSwapEvent(
            tx_ref=tx_hash,
...
def test_gwei_names_set_address(ethereum_inquirer, ethereum_accounts):
    ...
    assert events == [
        EvmEvent(
            ...
            event_subtype=HistoryEventSubType.FEE,
            asset=A_ETH,
            amount=FVal('0.00000977437994491'),
            location_label=(user_address := ethereum_accounts[0]),
            counterparty=CPT_GAS,
        ), EvmEvent(
            tx_ref=tx_hash,
...
def test_gwei_names_set_contenthash(ethereum_inquirer, ethereum_accounts):
    ...
    assert events == [
        EvmEvent(
            ...
            event_subtype=HistoryEventSubType.FEE,
            asset=A_ETH,
            amount=FVal('0.000031849482034734'),
            location_label=(user_address := ethereum_accounts[0]),
            counterparty=CPT_GAS,
        ), EvmEvent(
            tx_ref=tx_hash,
...
def test_gwei_names_set_text(ethereum_inquirer, ethereum_accounts):
    ...
    assert events == [
        EvmEvent(
            ...
            event_subtype=HistoryEventSubType.FEE,
            asset=A_ETH,
            amount=FVal('0.0000104263399416'),
            location_label=(user_address := ethereum_accounts[0]),
            counterparty=CPT_GAS,
        ), EvmEvent(
            tx_ref=tx_hash,
...
def test_gwei_names_renew(ethereum_inquirer, ethereum_accounts):
    ...
    assert events == [
        EvmEvent(
            ...
            event_subtype=HistoryEventSubType.FEE,
            asset=A_ETH,
            amount=FVal('0.000007733653032468'),
            location_label=(user_address := ethereum_accounts[0]),
            counterparty=CPT_GAS,
        ), EvmEvent(
            tx_ref=tx_hash,
...
def test_gwei_names_set_primary_name(ethereum_inquirer, ethereum_accounts):
    ...
    assert events == [
        EvmEvent(
            ...
            event_subtype=HistoryEventSubType.FEE,
            asset=A_ETH,
            amount=FVal('0.000007272510321351'),
            location_label=(user_address := ethereum_accounts[0]),
            counterparty=CPT_GAS,
        ), EvmEvent(
            tx_ref=tx_hash,
...
def test_gwei_names_register_subdomain(ethereum_inquirer, ethereum_accounts):
    ...
    assert events == [
        EvmEvent(
            ...
            event_subtype=HistoryEventSubType.FEE,
            asset=A_ETH,
            amount=FVal('0.000021935730743095'),
            location_label=(user_address := ethereum_accounts[0]),
            counterparty=CPT_GAS,
        ), EvmEvent(
            tx_ref=tx_hash,
