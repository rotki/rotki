...
from rotkehlchen.db.internal_tx_conflicts import (
    ...
    INTERNAL_TX_CONFLICT_REDECODE_REASON_MIXED_ZERO_GAS,
    INTERNAL_TX_CONFLICT_REDECODE_REASON_MIXED_ZERO_GAS_AND_DUPLICATE,
...
def test_migration_24(database: DBHandler) -> None:
    ...
    rotki = MockRotkiForMigrations(database)
    dbevmtx = DBEvmTx(database)
    tx_hash_1, tx_hash_2, tx_hash_3, tx_hash_4 = (
        make_evm_tx_hash(),
        make_evm_tx_hash(),
        make_evm_tx_hash(),
        make_evm_tx_hash(),
    )
    tx_1_sender, tx_2_sender, tx_3_sender, tx_4_sender = (
        make_evm_address(),
        make_evm_address(),
        make_evm_address(),
        make_evm_address(),
    ...
    evm_transactions = [
        EvmTransaction(
            tx_hash=tx_hash,
            chain_id=ChainID.ETHEREUM,
            timestamp=Timestamp(1700000000 + index),
        ...
        for index, (tx_hash, sender) in enumerate([
            (tx_hash_1, tx_1_sender),
            (tx_hash_2, tx_2_sender),
            (tx_hash_3, tx_3_sender),
            (tx_hash_4, tx_4_sender),
        ])
    ]
    ...
    with database.user_write() as write_cursor:
        ...
        write_cursor.executemany(
            ...
            [
                ...
                (tx_2_id, 0, tx_2_sender, (tx_2_to := make_evm_address()), '7', '0', '0'),
                (tx_2_id, 1, tx_2_sender, tx_2_to, '7', '0', '0'),
                (tx_3_id, 0, tx_3_sender, (tx_3_to := make_evm_address()), '9', '0', '0'),
                (tx_3_id, 1, tx_3_sender, tx_3_to, '9', '12345', '99'),
                (tx_4_id, 0, tx_4_sender, (tx_4_to := make_evm_address()), '11', '69640', '0'),
                (tx_4_id, 1, tx_4_sender, tx_4_to, '11', '69640', '0'),
            ],
        )
        write_cursor.executemany(
            'INSERT INTO evm_tx_mappings(tx_id, value) VALUES (?, ?)',
        ...
        write_cursor.execute(
            ...
            (
                ...
                A_ETH.identifier,
                '2',
                'to be deleted during fix_redecode',
                HistoryEventType.TRADE.serialize(),
                HistoryEventSubType.SPEND.serialize(),
                None,
                0,
    ...
    with database.conn.read_ctx() as cursor:
        assert set(cursor.execute(
            'SELECT chain, transaction_hash, action, repull_reason, redecode_reason, fixed FROM evm_internal_tx_conflicts '  # noqa: E501
            'ORDER BY transaction_hash',
        ).fetchall()) == {
            (ChainID.ETHEREUM.serialize_for_db(), bytes(tx_hash_1), INTERNAL_TX_CONFLICT_ACTION_FIX_REDECODE, None, INTERNAL_TX_CONFLICT_REDECODE_REASON_MIXED_ZERO_GAS_AND_DUPLICATE, 1),  # noqa: E501
            (
            ...
            (ChainID.ETHEREUM.serialize_for_db(), bytes(tx_hash_3), INTERNAL_TX_CONFLICT_ACTION_FIX_REDECODE, None, INTERNAL_TX_CONFLICT_REDECODE_REASON_MIXED_ZERO_GAS, 0),  # noqa: E501
        }
        assert cursor.execute(
