...
def inquirer_inject_evm_managers_set_order(
    ...
    if add_defi_oracles is True:
        uniswap_v2_oracle = UniswapV2Oracle(database=evm_managers[0].node_inquirer.database)
        uniswap_v3_oracle = UniswapV3Oracle(database=evm_managers[0].node_inquirer.database)
