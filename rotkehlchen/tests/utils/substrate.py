...
# Use multiple public nodes for tests
KUSAMA_TEST_NODES = (
...
def attempt_connect_test_nodes(
    ...
    def attempt_connect_node(node: NodeName) -> tuple[NodeName, NodeNameAttributes | None]:
        ...
        if chain == SupportedBlockchain.KUSAMA and node == KusamaNodeName.ONFINALITY:
            endpoints.append(KUSAMA_TEST_RPC_ENDPOINT)
        ...
        for endpoint in endpoints:
            try:
                ...
                message = (
                    ...
                    f'endpoint: {endpoint}. Connection error: {e!s}.'
                )
                log.error(message)
                continue

            log.info(f'Substrate tests connected to {node} node at endpoint: {node_interface.url}.')  # noqa: E501
    ...
    for attempt in range(retries):
        ...
        if available_node_attributes_map:
            return available_node_attributes_map

        not_connected_nodes = set(node_names)
        log.info(
