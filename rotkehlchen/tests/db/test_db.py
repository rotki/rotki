...
def test_writing_fetching_data(
    ...
    with data.db.user_write() as write_cursor:
        ...
        data.db.add_blockchain_accounts(
            write_cursor,
            [
                BlockchainAccountData(chain=SupportedBlockchain.ETHEREUM, address=string_to_evm_address('0xd36029d76af6fE4A356528e4Dc66B2C18123597D')),  # noqa: E501
...
def test_unlock_with_invalid_premium_data(
    ...
    data.unlock(username, '123', create_new=True, resume_from_backup=False)
    with data.db.conn.read_ctx() as write_cursor:
