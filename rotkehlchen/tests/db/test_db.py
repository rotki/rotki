...
def test_data_init_and_password(
        data_dir,
        username,
        messages_aggregator,
...
def test_writing_fetching_data(
    ...
    with data.db.user_write() as write_cursor:
        ...
        data.db.add_blockchain_accounts(
            ...
            [
                ...
                BlockchainAccountData(chain=SupportedBlockchain.ETHEREUM, address=string_to_evm_address('0x80B369799104a47e98A553f3329812a44A7FaCDc')),  # noqa: E501
                # Add this to 2 evm chains
                BlockchainAccountData(chain=SupportedBlockchain.ETHEREUM, address=string_to_evm_address('0x2B888954421b424C5D3D9Ce9bB67c9bD47537d12')),  # noqa: E501
                BlockchainAccountData(chain=SupportedBlockchain.POLYGON_POS, address=string_to_evm_address('0x2B888954421b424C5D3D9Ce9bB67c9bD47537d12')),  # noqa: E501
            ],
        )
    ...
    with data.db.conn.read_ctx() as cursor:
        for cache_key, kargs, value in cache_data:  # set and ensure value is set for each key
            with data.db.user_write() as write_cursor:
                data.db.set_dynamic_cache(write_cursor, cache_key, value, **kargs)  # type: ignore
            assert data.db.get_dynamic_cache(cursor, cache_key, **kargs) == value  # type: ignore
    ...
    with data.db.conn.read_ctx() as cursor:
        ...
        for cache_key, kargs, _ in cache_data:  # check cache stuff got deleted
            assert data.db.get_dynamic_cache(cursor, cache_key, **kargs) is None  # type: ignore
...
def test_settings_entry_types(database: 'DBHandler'):
    with database.user_write() as write_cursor:
        database.set_settings(write_cursor, ModifiableDBSettings(
...
def test_balance_save_frequency_check(
        data_dir: Path,
        username: str,
        messages_aggregator: MessagesAggregator,
        sql_vm_instructions_cb: int,
...
def test_query_collection_timed_balances(
        data_dir: Path,
        username: str,
        sql_vm_instructions_cb: int,
        messages_aggregator: MessagesAggregator,
...
def test_timed_balances_inferred_zero_balances(
        data_dir: Path,
        username: str,
        sql_vm_instructions_cb: int,
        messages_aggregator: MessagesAggregator,
...
def test_get_latest_location_value_distribution(
        data_dir: Path,
        username: str,
        sql_vm_instructions_cb: int,
        messages_aggregator: MessagesAggregator,
...
def test_get_latest_asset_value_distribution(
        data_dir: Path,
        username: str,
        sql_vm_instructions_cb: int,
        messages_aggregator: MessagesAggregator,
...
def test_get_netvalue_data_from_date(
        data_dir: Path,
        username: str,
        sql_vm_instructions_cb: int,
        messages_aggregator: MessagesAggregator,
...
def test_get_netvalue_without_nfts(
        data_dir: Path,
        username: str,
        sql_vm_instructions_cb: int,
        messages_aggregator: MessagesAggregator,
...
def test_add_trades(
    ...
    trade2 = Trade(
        timestamp=Timestamp(1451607500),
        location=Location.BINANCE,
        base_asset=A_BTC,
        quote_asset=A_ETH,
        trade_type=TradeType.BUY,
        amount=AssetAmount(FVal('0.00120')),
        rate=Price(FVal('10')),
        fee=Fee(FVal('0.001')),
        fee_currency=A_ETH,
        link='',
    ...
    trade3 = Trade(
        timestamp=Timestamp(1451608600),
        location=Location.COINBASE,
        base_asset=A_BTC,
        quote_asset=A_ETH,
...
def test_add_margin_positions(
    ...
    margin1 = MarginPosition(
        ...
        profit_loss=AssetAmount(FVal('1.0')),
        pl_currency=A_BTC,
        fee=Fee(FVal('0.01')),
        fee_currency=A_EUR,
        link='',
    ...
    margin3 = MarginPosition(
        ...
        profit_loss=AssetAmount(FVal('2.5')),
        pl_currency=A_BTC,
        fee=Fee(FVal('0.01')),
        fee_currency=A_EUR,
        link='',
...
def test_can_unlock_db_with_disabled_taxfree_after_period(
        data_dir: Path,
        username: str,
        sql_vm_instructions_cb: int,
        messages_aggregator: MessagesAggregator,
...
def test_timed_balances_primary_key_works(
    ...
    with db.conn.read_ctx() as cursor:
        timed_balances = db.query_timed_balances(cursor, asset=A_BTC, balance_type=BalanceType.ASSET)  # noqa: E501
        assert len(timed_balances) == 0
        balances = [
            DBAssetBalance(
                category=BalanceType.ASSET,
                time=Timestamp(1590676728),
    ...
    with db.user_write() as write_cursor:
        db.add_multiple_balances(write_cursor, balances)
...
def test_timed_balances_treat_eth2_as_eth(database):
    ...
    with database.user_write() as write_cursor:
        database.set_settings(write_cursor, ModifiableDBSettings(infer_zero_timed_balances=True))
        database.add_multiple_balances(write_cursor, balances)
    with database.conn.read_ctx() as cursor:
...
def test_multiple_location_data_and_balances_same_timestamp(
    ...
    with db.conn.read_ctx() as cursor:
        timed_balances = db.query_timed_balances(cursor=cursor, from_ts=Timestamp(0), to_ts=Timestamp(1590676728), asset=A_BTC, balance_type=BalanceType.ASSET)  # noqa: E501
        assert len(timed_balances) == 0
...
def test_set_get_rotkehlchen_premium_credentials(
        data_dir: Path,
        username: str,
        sql_vm_instructions_cb: int,
        messages_aggregator: MessagesAggregator,
...
def test_unlock_with_invalid_premium_data(
        data_dir: Path,
        username: str,
        sql_vm_instructions_cb: int,
        messages_aggregator: MessagesAggregator,
...
def test_remove_queried_address_on_account_remove(
        data_dir: Path,
        username: str,
        sql_vm_instructions_cb: int,
        messages_aggregator: MessagesAggregator,
    ...
    with data.db.conn.read_ctx() as cursor:
        ...
        assert string_to_evm_address('0xd36029d76af6fE4A356528e4Dc66B2C18123597D') in addresses
...
def test_fresh_db_adds_version(
        user_data_dir: Path,
        username: str,
        sql_vm_instructions_cb: int,
        messages_aggregator: MessagesAggregator,
...
def test_db_add_skipped_external_event_twice(database: 'DBHandler') -> None:
    ...
    with database.conn.read_ctx() as cursor:
        assert cursor.execute('SELECT COUNT(*) FROM skipped_external_events').fetchone()[0] == 1
