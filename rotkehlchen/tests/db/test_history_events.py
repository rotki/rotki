...
def _make_paginated_filter(limit: int, offset: int) -> HistoryEventFilterQuery:
...
def test_pagination_with_entries_limit(database: DBHandler) -> None:
    ...
    with database.conn.read_ctx() as cursor:
        ...
        assert [(num, event.group_identifier) for num, event in history_events.get_history_events(
            cursor=cursor,
            filter_query=_make_paginated_filter(limit=2, offset=0),
            entries_limit=5,
            aggregate_by_group_ids=True,
        )] == [(2, 'group_10'), (2, 'group_9')]
        assert [(num, event.group_identifier) for num, event in history_events.get_history_events(
            cursor=cursor,
            filter_query=_make_paginated_filter(limit=2, offset=4),  # page straddling the cap
            entries_limit=5,
            aggregate_by_group_ids=True,
        )] == [(2, 'group_6')]
        assert history_events.get_history_events(
            cursor=cursor,
            filter_query=_make_paginated_filter(limit=2, offset=6),  # page beyond the cap
            entries_limit=5,
            aggregate_by_group_ids=True,
        ) == []
        ...
        assert [event.group_identifier for event in history_events.get_history_events(
            cursor=cursor,
            filter_query=_make_paginated_filter(limit=4, offset=8),
            entries_limit=5,
            aggregate_by_group_ids=False,
        )] == ['group_6', 'group_6']
        assert history_events.get_history_events(
            cursor=cursor,
            filter_query=_make_paginated_filter(limit=4, offset=12),
            entries_limit=5,
            aggregate_by_group_ids=False,
        ) == []
