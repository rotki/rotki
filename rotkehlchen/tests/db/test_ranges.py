...
def test_update_used_query_range(database):
    ...
    with database.conn.read_ctx() as cursor, database.user_write() as write_cursor:
        ...
        start_ts = 12
        end_ts = 90
        query_range = dbranges.get_location_query_ranges(write_cursor, location1, start_ts, end_ts)
        ...
        assert database.get_used_query_range(write_cursor, location1) == (12, 90)

        start_ts = 250
        end_ts = 500
        query_range = dbranges.get_location_query_ranges(write_cursor, location2, start_ts, end_ts)
        dbranges.update_used_query_range(
            cursor,
            write_cursor,
            location2,
        ...
        assert database.get_used_query_range(write_cursor, location2) == (10, 500)
