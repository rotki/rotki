...
from rotkehlchen.constants.assets import A_COW, A_ETH, A_XRP
...
def test_upgrade_db_47_to_48(user_data_dir, messages_aggregator):
    ...
    with db_v47.conn.read_ctx() as cursor:
        ...
        assert A_XRP.identifier in db_v47.get_ignored_asset_ids(cursor)
    ...
    with db.conn.read_ctx() as cursor:
        ...
        for asset, ignored in cursor.execute('SELECT asset, ignored FROM history_events'):
            assert ignored == (1 if asset == A_XRP.identifier else 0)
