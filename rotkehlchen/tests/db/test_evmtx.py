...
def test_add_get_evm_transactions(
        data_dir: Path,
        username: str,
    ...
    with data.db.conn.read_ctx() as cursor:
        ...
        errors = messages_aggregator.consume_errors()
        warnings = messages_aggregator.consume_warnings()
        assert len(errors) == 0
        ...
        result = dbevmtx.get_evm_transactions(cursor, EvmTransactionsFilterQuery.make(tx_hash=deserialize_evm_tx_hash(b'dsadsad'), chain_id=ChainID.ETHEREUM), has_premium=True)  # noqa: E501
        assert result == []
...
def test_query_also_internal_evm_transactions(
        data_dir: Path,
        username: str,
        sql_vm_instructions_cb: int,
        messages_aggregator: MessagesAggregator,
    ...
    with data.db.conn.read_ctx() as cursor:
        with data.db.user_write() as write_cursor:
            dbevmtx.add_evm_transactions(write_cursor, [tx1, tx3, tx4, tx5], relevant_address=ETH_ADDRESS1)  # noqa: E501
            dbevmtx.add_evm_transactions(write_cursor, [tx2], relevant_address=ETH_ADDRESS2)
            dbevmtx.add_evm_transactions(write_cursor, [tx1, tx3], relevant_address=ETH_ADDRESS3)
            dbevmtx.add_evm_internal_transactions(write_cursor, [internal_tx2, internal_tx3, internal_tx4], relevant_address=ETH_ADDRESS1)  # noqa: E501
            dbevmtx.add_evm_internal_transactions(write_cursor, [internal_tx1, internal_tx4], relevant_address=ETH_ADDRESS3)  # noqa: E501
            dbevmtx.add_evm_internal_transactions(write_cursor, [internal_tx1], relevant_address=address_4)  # noqa: E501
        errors = messages_aggregator.consume_errors()
        warnings = messages_aggregator.consume_warnings()
        assert len(errors) == 0
        assert len(warnings) == 0
