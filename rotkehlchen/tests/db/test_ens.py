...
def _simple_ens_setup(database, freezer) -> Tuple[DBEns, ChecksumEthAddress, ChecksumEthAddress]:
    ...
    dbens.add_ens_mapping(addy1, 'addy1', ts_now())
    dbens.add_ens_mapping(addy2, 'addy2', ts_now())
    mappings = dbens.get_reverse_ens([addy1, addy2])
    assert mappings == {
        addy1: EnsMapping(
            address=addy1,
            name='addy1',
...
def test_update_ens_mapping(database, freezer):
    ...
    dbens.add_ens_mapping(addy1, None, ts_now())
    mappings = dbens.get_reverse_ens([addy1, addy2])
    assert mappings == {
        addy1: ts2,
        addy2: EnsMapping(
    ...
    dbens.add_ens_mapping(addy1, 'new addy1 name', ts_now())
    mappings = dbens.get_reverse_ens([addy1, addy2])
    assert mappings == {
        addy1: EnsMapping(
            address=addy1,
    ...
    dbens.add_ens_mapping(addy2, 'new addy2 name', ts_now())
    mappings = dbens.get_reverse_ens([addy1, addy2])
    assert mappings == {
        addy1: EnsMapping(
            address=addy1,
...
def test_multiple_ens_mapping_none(database, freezer):
    ...
    ts3 = START_TS + 3600
    freezer.move_to(datetime.fromtimestamp(ts3))
    dbens.add_ens_mapping(addy2, None, ts_now())
    mappings = dbens.get_reverse_ens([addy1, addy2])
    assert mappings == {
        addy1: ts2,
        addy2: ts3,
...
def test_conflict(database, freezer):
    ...
    dbens.add_ens_mapping(addy1, 'addy1', ts_now())  # adding existing mapping == noop
    ...
    with pytest.raises(sqlcipher.IntegrityError):  # pylint: disable=no-member
        dbens.add_ens_mapping(addy3, 'addy1', ts_now())
    mappings = dbens.get_reverse_ens([addy1, addy2, addy3])
    assert mappings == {
        addy1: EnsMapping(
            address=addy1,
            name='addy1',
