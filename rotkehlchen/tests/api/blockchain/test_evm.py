...
def test_add_multievm_accounts(rotkehlchen_api_server: APIServer) -> None:
    ...
    with ExitStack() as stack:
        setup_evm_addresses_activity_mock(
            ...
            monad_addresses=[common_account],
...
def test_detect_evm_accounts(
    ...
    with ExitStack() as stack:
        ...
        stack.enter_context(patch.object(
            rotki.chains_aggregator.monad.node_inquirer,
            'has_activity',
            return_value=HasChainActivity.NONE,
        ))
        stack.enter_context(patch.object(
            rotki.chains_aggregator.sonic.node_inquirer,
            'has_activity',
            return_value=HasChainActivity.NONE,
        ))
        stack.enter_context(patch.object(
...
def test_adding_safe(
    ...
    assert result == {
        ...
        'failed': {safe_address: ['binance_sc']},
    }
...
def test_evm_account_addition_preserves_labels_across_chains(rotkehlchen_api_server: APIServer) -> None:  # noqa: E501
    ...
    with ExitStack() as stack:
        ...
        result = assert_proper_sync_response_with_result(response)
        assert result == {
            'added': {addy: ['optimism', 'polygon_pos', 'arbitrum_one', 'base', 'hyperliquid', 'gnosis', 'scroll', 'binance_sc', 'monad', 'sonic', 'robinhood']},  # noqa: E501
            'existed': {addy: ['eth']},
        }
