...
@pytest.mark.vcr(filter_query_parameters=['apikey'])  # This test doesn't have any recorded cassette. VCR is used here to ensure that the mocking logic works properly with no external requests made, otherwise a cassette error would be raised.  # noqa: E501
@pytest.mark.parametrize('have_decoders', [True])
@pytest.mark.parametrize(
    'added_exchanges',
    [(Location.BINANCE, Location.POLONIEX, Location.BITMEX, Location.KRAKEN)],
...
@pytest.mark.parametrize('have_decoders', [True])
@pytest.mark.parametrize('ethereum_accounts', [[]])  # no accounts so no history is pulled
@pytest.mark.parametrize('mocked_price_queries', [prices])
