...
UNIQUE_CACHE_KEYS = ['TEST_KEY']
''' UNIQUE_CACHE_KEYS: set[GeneralCacheType] = {GeneralCacheType.CURVE_POOL_ADDRESS,
    GeneralCacheType.MAKERDAO_VAULT_ILK,GeneralCacheType.CURVE_GAUGE_ADDRESS}'''
...
def compute_cache_key(key_parts: Iterable[Union[str, GeneralCacheType]]) -> tuple[str, str]:
    ...
    cache table the data must me stored in."""
    unique_cache = list(key_parts)[0] in UNIQUE_CACHE_KEYS
    cache_table = UNIQUE_CACHE if unique_cache else GENERAL_CACHE
    cache_key = ''
...
def globaldb_delete_cache(
        write_cursor: DBCursor,
        key_parts: Iterable[Union[str, GeneralCacheType]],
        value: Optional[str] = None,
) -> None:
    """Deletes from cache in globaldb. In the case of the general cache,
    If value is None deletes all entries with the provided cache key.
    Otherwise, deletes only single entry `key - value`."""
    cache_key, cache_table = compute_cache_key(key_parts)
    if value is None:
        write_cursor.execute(f'DELETE FROM {cache_table} WHERE key=?', (cache_key,))
    else:
        write_cursor.execute('DELETE FROM general_cache WHERE key=? AND value=?', (cache_key, value))  # noqa: E501


def globaldb_delete_cache_like(
        write_cursor: DBCursor,
        key_parts: Iterable[Union[str, GeneralCacheType]],
) -> None:
    """
    ...
    """
    cache_key, cache_table = compute_cache_key(key_parts)
    write_cursor.execute(f'DELETE FROM {cache_table} WHERE key LIKE ?', (f'{cache_key}%',))
