...
def asset_from_hyperliquid(symbol: str) -> Asset:
    """Fetch asset from hperliquid location
...
class Hyperliquid:
    ...
    def _query(
        ...
        try:
            ...
            log.error(
                f'Query to hyperliquid for {address} and {account_type=} failed due to '
                f'{e}. Skipping',
            )
            raise RemoteError(
                f'Failed to query balances of {account_type} for {address} due to {e}',
            ) from e
        ...
        if response.status_code != HTTPStatus.OK:
            ...
            raise RemoteError(f'Failed to query balances of {account_type} for {address} with status code {response.status_code}')  # noqa: E501
    ...
    def query_balances(self, address: ChecksumEvmAddress) -> dict[Asset, FVal]:
        ...
        Hyperliquid has like two `accounts` for each address, one for spot and one for perpetuals.
        ...
        for asset_entry in data.get('balances', []):
            ...
            except (DeserializationError, UnknownAsset, WrongAssetType, KeyError) as e:
        ...
        try:
            ...
            try:
                ...
                log.error(f'Failed to read crossMarginSummary in hyperliquid due to {e}')
