...
class Blockscout(ExternalServiceWithApiKey, EtherscanLikeApi):
    ...
    def _get_url(self, chain_id: ChainID) -> str:
        match chain_id:
            ...
            case _:
                raise NotImplementedError(f'Blockscout not implement for {chain_id.name}')
    ...
    def has_activity(self, account: ChecksumEvmAddress) -> HasChainActivity:
        ...
        result = self._query(chain_id=self.chain_id, module='account', action='txlist', options=options)
        if len(result) != 0:
            return HasChainActivity.TRANSACTIONS

        result = self._query(chain_id=self.chain_id, module='account', action='txlistinternal', options=options)
        if len(result) != 0:
            return HasChainActivity.TRANSACTIONS

        result = self._query(chain_id=self.chain_id, module='account', action='tokentx', options=options)
        if len(result) != 0:
