...
class RestAPI:
    ...
    def get_historical_prices_per_asset(
        ...
        # Generate timestamps for each interval between from_timestamp and to_timestamp
        # Filter out any timestamps that are in the exclude_timestamps or in the future
        timestamps = [
            ts for i in range((to_timestamp - from_timestamp) // interval + 1)
            if (ts := Timestamp(from_timestamp + (i * interval))) not in exclude_timestamps and ts <= ts_now()  # noqa: E501
        ]
