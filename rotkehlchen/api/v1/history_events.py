...
async def get_history_events(
    # Filters
    event_identifiers: Optional[list[str]] = Query(None),
    event_types: Optional[list[str]] = Query(None),
    locations: Optional[list[str]] = Query(None),
    assets: Optional[list[str]] = Query(None),
    from_timestamp: Optional[int] = Query(None, ge=0),
    to_timestamp: Optional[int] = Query(None, ge=0),
    # Pagination
    limit: int = Query(500, ge=1, le=10000),
    offset: int = Query(0, ge=0),
    # Ordering
    order_by_fields: Optional[list[str]] = Query(None),
    order_by_ascending: Optional[list[bool]] = Query(None),
    # Async query
    ...
    try:
        ...
        if async_query:
            ...
            task = await task_manager.spawn_and_track(
                task_name=f'query_history_events',
                coro=_query_events(),
                timeout=120.0,
            )
