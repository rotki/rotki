...
def _to_fval(value: Any) -> FVal:
    return FVal(value) if value is not None else FVal(0)
...
class DBLidoCsm:
    ...
    def _deserialize_metrics_row(row: tuple[Any, ...]) -> LidoCsmNodeOperatorStats | None:
        ...
        return LidoCsmNodeOperatorStats(
            ...
            operator_type_label=operator_type.label if operator_type is not None else 'Unknown',
            current_bond=_to_fval(bond_current),
            required_bond=_to_fval(bond_required),
            claimable_bond=_to_fval(bond_claimable),
            total_deposited_keys=int(total_deposited_keys)
            if total_deposited_keys is not None
            else 0,
            rewards_steth=_to_fval(rewards_pending),
        )
