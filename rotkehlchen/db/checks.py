...
def sanity_check_impl_sync(
        cursor: 'sqlite3.Cursor',
        db_name: str,
        minimized_schema: dict[str, str],
) -> None:
    """Synchronous version of sanity_check_impl for backward compatibility"""
    ...
    extra_db_structures = cursor.fetchall()
    if len(extra_db_structures) > 0:
        logger.critical(
            f'Unexpected structures in {db_name} database: {extra_db_structures}',
        )
    ...
    if len(differing_tables_properties) > 0:
        log_msg = f'Differing tables in the {db_name} database are:'
        for table_name, ((raw_script, minimized_script), structure_expected) in differing_tables_properties.items():
            log_msg += (f'\n- For table {table_name} expected {structure_expected} but found {minimized_script}. Table raw script is: {raw_script}')
        logger.critical(log_msg)
        raise DBSchemaError(
            f'Structure of some tables in your {db_name} database differ from the '
            f'expected. Check the logs for more details. ' + DEFAULT_SANITY_CHECK_MESSAGE,
