...
class DBEvmTx():
    ...
    def purge_evm_transaction_data(self, chain: EVMChain) -> None:
    ...
    def get_transaction_hashes_not_decoded(
        ...
        """Get transaction hashes of not decoded transactions. Optionally by chain id"""
        cursor = self.db.conn.cursor()
        querystr = (
            'SELECT A.tx_hash from evmtx_receipts AS A LEFT OUTER JOIN evm_tx_mappings AS B '
            'ON A.tx_hash=B.tx_hash AND A.chain_id=B.chain_id WHERE B.tx_hash is NULL'
        )
        bindings = []
