...
class DBAccountingRules:
    ...
    def add_accounting_rule(
        ...
        with self.db.conn.write_ctx() as write_cursor:
            ...
            rowid = write_cursor.lastrowid

        with self.db.conn.read_ctx() as cursor:
            if (result := cursor.execute(
                'SELECT identifier FROM accounting_rules WHERE rowid=?',
                (rowid,),
            ).fetchone()) is not None:
                inserted_rule_id = result[0]
            else:
                raise InputError(
                    'Could not fetch inserted rule identifier for ' + self._rule_for_string(
