...
class DBHandler:
    ...
    def get_account_blockchains(
            self,
            cursor: 'DBCursor',
            account: AnyBlockchainAddress,
    ) -> list[SupportedBlockchain]:
        """Returns all blockchains for the specified account."""
        return [
            blockchain
            for entry in cursor.execute(
                'SELECT blockchain FROM blockchain_accounts WHERE account=?;',
                (account,),
            )
