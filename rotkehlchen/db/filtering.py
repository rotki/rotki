...
from rotkehlchen.typing import ChecksumEthAddress, Timestamp, BTCAddress
...
@dataclass(init=True, repr=True, eq=True, order=False, unsafe_hash=False, frozen=False)
class TransactionsFilterQuery(DBFilterQuery):
...
@dataclass(init=True, repr=True, eq=True, order=False, unsafe_hash=False, frozen=False)
class ETHTransactionsFilterQuery(TransactionsFilterQuery):
...
@dataclass(init=True, repr=True, eq=True, order=False, unsafe_hash=False, frozen=False)
class DBBTCTransactionHashFilter(DBFilter):
    tx_hash: Optional[Union[bytes, str]] = None
...
@dataclass(init=True, repr=True, eq=True, order=False, unsafe_hash=False, frozen=False)
class DBBTCTransactionAddressFilter(DBFilter):
    addresses: Optional[List[BTCAddress]] = None
...
@dataclass(init=True, repr=True, eq=True, order=False, unsafe_hash=False, frozen=False)
class BTCTransactionsFilterQuery(TransactionsFilterQuery):

    @property
    def address_filter(self) -> Optional[DBBTCTransactionAddressFilter]:
        if len(self.filters) >= 1 and isinstance(self.filters[0], DBBTCTransactionAddressFilter):
            return self.filters[0]
        return None

    @property
    def addresses(self) -> Optional[List[BTCAddress]]:
        address_filter = self.address_filter
        if address_filter is None:
            return None
        return address_filter.addresses
    ...
    @addresses.setter
    def addresses(self, addresses: Optional[List[BTCAddress]]) -> None:
        address_filter = self.address_filter
        if address_filter is None:
            return
        address_filter.addresses = addresses
    ...
    def make(
        ...
        filters: List[DBFilter] = []
        if tx_hash:  # tx_hash means single result so make it as single filter
            filters.append(DBBTCTransactionHashFilter(and_op=False, tx_hash=tx_hash))
        else:
            filters.append(DBBTCTransactionAddressFilter(and_op=False, addresses=addresses))
            filters.append(
                DBTimestampFilter(
                    and_op=True,
                    from_ts=from_ts,
        ...
        filter_query.filters = filters
        return cast('BTCTransactionsFilterQuery', filter_query)
