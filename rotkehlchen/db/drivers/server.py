...
class ZMQCallData:
    ...
    def __str__(self) -> str:
        return f'ZMQCallData(connection_type={self.connection_type}, db_path={self.db_path}, method={self.method}, args={self.args}, kwargs={self.kwargs})'  # noqa: E501
...
class DBWriterServer:
    ...
    def __init__(self) -> None:
        ...
        self.db_cursors: defaultdict[str, UnderlyingCursor | None] = defaultdict(None)
    ...
    def _handle_query(
        ...
        if call_data.use_cursor:
            starting_reference = self.db_cursors[call_data.db_path]
        ...
        while True:
            try:
                ...
                if call_data.method == '*open_cursor*':
                    self.db_cursors[call_data.db_path] = db_connection.cursor()
                    self.zmq_connection.send_pyobj(None)
                    continue
                if call_data.method == '*close_cursor*':
                    del self.db_cursors[call_data.db_path]
                    self.zmq_connection.send_pyobj(None)
                    continue
                ...
                if self.shutting_down:
                    break
                self.zmq_connection.send_pyobj(ZMQReturnData(error=e).serialize())
