...
def upgrade_v27_to_v28(db: 'DBHandler', progress_handler: 'DBUpgradeProgressHandler') -> None:
    ...
    with db.conn.read_ctx() as cursor, db.user_write() as write_cursor:
        ...
        write_cursor.execute('INSERT OR IGNORE INTO gitcoin_tx_type(type, seq) VALUES ("A", 1)')
        # ZKSync Transaction
        write_cursor.execute('INSERT OR IGNORE INTO gitcoin_tx_type(type, seq) VALUES ("B", 2)')

        write_cursor.execute("""CREATE TABLE IF NOT EXISTS ledger_actions_gitcoin_data (
        ...
        write_cursor.execute("""CREATE TABLE IF NOT EXISTS gitcoin_grant_metadata (
