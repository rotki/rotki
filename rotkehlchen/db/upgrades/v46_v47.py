...
def upgrade_v46_to_v47(db: 'DBHandler', progress_handler: 'DBUpgradeProgressHandler') -> None:
    ...
    def _remove_nft_collection_assets(write_cursor: 'DBCursor') -> None:
        ...
        # save it in a temporary table that will need dealt with manually.
        # Normally no data should be found here.
        selected_data = {}
        ...
        for table, columns in (
            ...
            where_clauses = ' OR '.join([f'{column} IN ({placeholders})' for column in columns])
            result = write_cursor.execute(
                f'SELECT * FROM {table} WHERE {where_clauses}',
                identifiers_to_remove * len(columns),
            ).fetchall()
            if len(result) > 0:
        ...
        if len(selected_data) > 0:
            ...
            write_cursor.executemany(
                ...
                [(table, json.dumps(data)) for table, data in selected_data.items()],
            )
        ...
        for table, columns in tuple(tables_to_delete_from) + (
            ...
            write_cursor.execute(
                f'DELETE FROM {table} WHERE {where_clauses}',
                identifiers_to_remove * len(columns),
            )
