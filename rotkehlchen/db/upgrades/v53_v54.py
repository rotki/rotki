...
def upgrade_v53_to_v54(db: DBHandler, progress_handler: DBUpgradeProgressHandler) -> None:
    ...
    @progress_step(description='Add Linea chain location.')
    def _add_linea_location(write_cursor: DBCursor) -> None:
        write_cursor.execute(
            'INSERT OR IGNORE INTO location(location, seq) VALUES (?, ?)',
            (Location.LINEA.serialize_for_db(), Location.LINEA.value),
        )
