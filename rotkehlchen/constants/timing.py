ETH_DAO_FORK_TS = 1469020840  # 2016-07-20 13:20:40 UTC
BTC_BCH_FORK_TS = 1501593374  # 2017-08-01 13:16:14 UTC
BCH_BSV_FORK_TS = 1542304352  # 2018-11-15 18:52:00 UTC

ROTKEHLCHEN_SERVER_TIMEOUT = 5
ROTKEHLCHEN_SERVER_BACKUP_TIMEOUT = ROTKEHLCHEN_SERVER_TIMEOUT * 30
GLOBAL_REQUESTS_TIMEOUT = 5  # perhaps consolidate this and the one above?

HOUR_IN_SECONDS = 3600
DAY_IN_SECONDS = 24 * HOUR_IN_SECONDS
WEEK_IN_SECONDS = DAY_IN_SECONDS * 7
MONTH_IN_SECONDS = WEEK_IN_SECONDS * 4
YEAR_IN_SECONDS = 31536000  # 60 * 60 * 24 * 365

# For queries that are attempted multiple times:
ENS_UPDATE_INTERVAL = DAY_IN_SECONDS

ETH_PROTOCOLS_CACHE_REFRESH = DAY_IN_SECONDS * 3
DATA_UPDATES_REFRESH = DAY_IN_SECONDS
EVM_ACCOUNTS_DETECTION_REFRESH = DAY_IN_SECONDS
ENS_AVATARS_REFRESH = DAY_IN_SECONDS
SPAM_ASSETS_DETECTION_REFRESH = HOUR_IN_SECONDS * 6
AUGMENTED_SPAM_ASSETS_DETECTION_REFRESH = DAY_IN_SECONDS
OWNED_ASSETS_UPDATE = HOUR_IN_SECONDS
