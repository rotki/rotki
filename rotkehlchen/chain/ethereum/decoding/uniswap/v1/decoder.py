...
class Uniswapv1Decoder(DecoderInterface):  # lgtm[py/missing-call-to-init]
        ...
        if tx_log.topics[0] == TOKEN_PURCHASE:
            ...
            for event in decoded_events:
                if event.event_type == HistoryEventType.SPEND and event.counterparty == buyer:
                    ...
                    out_event = event
                elif event.event_type == HistoryEventType.TRADE and event.event_subtype == HistoryEventSubType.RECEIVE and event.counterparty == CPT_UNISWAP:  # noqa: :E501
                    in_event = event
            ...
            for event in decoded_events:
                if event.event_type == HistoryEventType.RECEIVE and event.location_label == buyer:
                    ...
                    in_event = event
                elif event.event_type == HistoryEventType.TRADE and event.event_subtype == HistoryEventSubType.SPEND and event.counterparty == CPT_UNISWAP:  # noqa: :E501
                    out_event = event
    ...
    def event_settings(self, pot: 'AccountingPot') -> Dict[str, 'TxEventSettings']:
        ...
        return {
            get_tx_event_type_identifier(HistoryEventType.TRADE, HistoryEventSubType.SPEND, CPT_UNISWAP): TxEventSettings(  # noqa: E501
                taxable=True,
