...
class Compoundv3Decoder(DecoderInterface):
        ...
        if compound_token not in self.underlying_tokens:
            if len(compound_token.underlying_tokens) == 0:
                ...
                try:  # if not in cached mapping, fetch from DB and add it
                    self.underlying_tokens[compound_token] = EvmToken(ethaddress_to_identifier(
    ...
    def _decode_supply_or_withdraw_event(
        ...
        paired_event = None
        action_from_event_type, action_to_event_subtype, action_to_notes = None, None, None
        for event in context.decoded_events:
            if (
    ...
    def _correct_supply_event(
            self,
            decoded_events: list['EvmEvent'],
            transaction: 'EvmTransaction',  # pylint: disable=unused-argument
            all_logs: list['EvmTxReceiptLog'],  # pylint: disable=unused-argument
    ) -> list['EvmEvent']:
        """Post decoding function to correct supply event's receive amount in notes."""
        for event in decoded_events:
            if (
                event.event_type == HistoryEventType.RECEIVE and
                event.event_subtype == HistoryEventSubType.RECEIVE_WRAPPED and
                event.counterparty == CPT_COMPOUND_V3 and event.notes is not None
            ):
    ...
    def post_decoding_rules(self) -> dict[str, list[tuple[int, Callable]]]:
        return {CPT_COMPOUND_V3: [(0, self._correct_supply_event)]}
