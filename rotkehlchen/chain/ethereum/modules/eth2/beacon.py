...
class BeaconNode:
    ...
    @overload
    def query(self, endpoint: str, data: dict[str, Any] | None = None) -> list[dict]:
        ...

    def query(self, endpoint: str, data: dict[str, Any] | None = None) -> dict | list[dict]:
        """
        May raise:
        ...
        try:
            response = self.session.request(
                url=url,
                json=data,
                method='POST' if endpoint == 'eth/v1/beacon/states/head/validators' else 'GET',
    ...
    def query_chunked(
            ...
            endpoint: Literal['eth/v1/beacon/states/head/validators'],
            chunk_size: Literal[80, 100] = DEFAULT_VALIDATOR_CHUNK_SIZE,
    ) -> list[dict]:
        """
        May raise:
        - RemoteError due to problems querying the node
