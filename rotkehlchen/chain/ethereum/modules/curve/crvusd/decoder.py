...
class CurvecrvusdDecoder(CurveBorrowRepayCommonDecoder, ReloadableDecoderMixin):
    ...
    def reload_data(self) -> Mapping['ChecksumEvmAddress', tuple[Any, ...]] | None:
        ...
        if should_update_protocol_cache(CacheType.CURVE_CRVUSD_CONTROLLERS) is True:
    ...
    def _decode_peg_keeper_update(self, context: DecoderContext) -> DecodingOutput:
        if context.tx_log.topics[0] != PEG_KEEPER_WITHDRAW_TOPIC:
            return DEFAULT_DECODING_OUTPUT

        # The amount in the peg keeper withdraw event is the total amount of crvUSD being traded.
        # The user is only rewarded with a small portion of that and is rewarded in a different
        # token. Unfortunately there isn't any information about the actual reward token and amount
        # in the withdraw tx_log, so we have to retrieve this info by inspecting the other tx_logs.
        crvusd_amount = int.from_bytes(context.tx_log.data[:32])
        reward_token_address = None
        for tx_log in context.all_logs:
            if tx_log.topics[0] != ERC20_OR_ERC721_TRANSFER:
                continue

            if (  # Transfer of crvUSD to the peg keeper matching amount in the withdraw event
                tx_log.address == self.crvusd.evm_address and
                bytes_to_address(tx_log.topics[2]) in CRVUSD_PEG_KEEPERS and
                crvusd_amount == int.from_bytes(tx_log.data[:32])
            ):
                reward_token_address = bytes_to_address(tx_log.topics[1])
            elif (  # Transfer of reward token from peg keeper to user wallet
                tx_log.address == reward_token_address and
                bytes_to_address(tx_log.topics[1]) in CRVUSD_PEG_KEEPERS and
                self.base.is_tracked(bytes_to_address(tx_log.topics[2]))
            ):
                reward_raw_amount = int.from_bytes(tx_log.data[:32])
                break
        else:
            log.error(f'Failed to find reward token and amount for curve peg keeper update transaction {context.transaction!s}')  # noqa: E501
            return DEFAULT_DECODING_OUTPUT

        reward_token = self.base.get_or_create_evm_token(address=reward_token_address)
        reward_amount = token_normalized_value(
        ...
        return DecodingOutput(action_items=[
            ActionItem(
                ...
                to_notes=f'Receive {reward_amount} {reward_token.symbol} after updating a Curve peg keeper',  # noqa: E501
            ),
        ])
    ...
    def addresses_to_decoders(self) -> dict[ChecksumEvmAddress, tuple[Any, ...]]:
        return (
            ...
            dict.fromkeys(CRVUSD_PEG_KEEPERS, (self._decode_peg_keeper_update,))
        )
