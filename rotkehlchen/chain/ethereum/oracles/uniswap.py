...
from .constants import (
    ...
    UNISWAPV2_FACTORY_ADDRESSES,
    UNISWAPV3_FACTORY_ADDRESSES,
)
...
class UniswapOracle(HistoricalPriceOracleInterface, CacheableMixIn):
    ...
    """
    def __init__(self, evm_inquirer: 'EvmNodeInquirer', version: int):
    ...
    def resolve_assets_and_load_inquirer(
            self,
            from_asset: Asset,
            to_asset: Asset,
    ) -> Sequence[EvmToken]:
        """Resolve assets to EvmTokens and load the correct evm_inquirer for the tokens chain.
        Returns a tuple containing the from token and to token, or None on error.
        """
        ...
        for asset in [from_asset, to_asset]:
            ...
            if (
                ...
                token.chain_id not in {ChainID.ETHEREUM, ChainID.POLYGON_POS, ChainID.ARBITRUM_ONE, ChainID.OPTIMISM, ChainID.BASE, ChainID.BINANCE_SC}  # noqa: E501
            ):
        ...
        if from_token.chain_id != to_token.chain_id and A_WETH.resolve_to_evm_token() in tokens:
            if from_asset == A_ETH:
                from_token = CHAIN_TO_WRAPPED_TOKEN[to_token.chain_id.to_blockchain()].resolve_to_evm_token()  # noqa: E501
            elif to_asset == A_ETH:
                to_token = CHAIN_TO_WRAPPED_TOKEN[from_token.chain_id.to_blockchain()].resolve_to_evm_token()  # noqa: E501

        # Load the evm inquirer and factory contract for this chain
        if self.evm_inquirer.chain_id != (chain_id := from_token.chain_id):
            self.evm_inquirer = Inquirer().get_evm_manager(chain_id=chain_id).node_inquirer
            self.update_factory_contract()
    ...
    @abc.abstractmethod
    def update_factory_contract(self) -> None:
        """Reload the factory contract depending on the chain id of the evm_inquirer."""
    ...
    def find_route(self, from_asset: EvmToken, to_asset: EvmToken) -> list[str]:
        ...
        """
        routing_assets = self.routing_assets[self.evm_inquirer.chain_id]
        output: list[str] = []
    ...
    def query_current_price(
        ...
        """
        from_token, to_token = self.resolve_assets_and_load_inquirer(
    ...
    def _call_methods(
        ...
        if (
            ...
            (isinstance(block_identifier, int) and block_identifier > self.evm_inquirer.contract_multicall.deployed_block)  # noqa: E501
        ):
            response = self.evm_inquirer.multicall(
                calls=[
                    (
            ...
            output = {
                method_name: self.evm_inquirer.call_contract(
    ...
    def can_query_history(
        ...
        self.resolve_assets_and_load_inquirer(
            from_asset=from_asset,
            to_asset=to_asset,
        )
        return self.is_before_contract_creation(self.evm_inquirer.get_blocknumber_by_time(timestamp))  # noqa: E501
    ...
    def query_historical_price(
        ...
        return self.get_price(
            from_token=from_token,
            to_token=to_token,
            block_identifier=self.evm_inquirer.get_blocknumber_by_time(timestamp),
            timestamp=timestamp,
        )
...
class UniswapV3Oracle(UniswapOracle):
    ...
    def __init__(self, evm_inquirer: 'EvmNodeInquirer'):
        super().__init__(evm_inquirer=evm_inquirer, version=3)
        self.uniswap_v3_pool_abi = self.evm_inquirer.contracts.abi('UNISWAP_V3_POOL')
        self.update_factory_contract()

    def update_factory_contract(self) -> None:
        self.uniswap_v3_factory = self.evm_inquirer.contracts.contract(UNISWAPV3_FACTORY_ADDRESSES[self.evm_inquirer.chain_id])  # noqa: E501
    ...
    def get_pool(
        ...
        for query in result:
            ...
            pool_contract = EvmContract(
                ...
                deployed_block=self.uniswap_v3_factory.deployed_block,
            )
    ...
    def is_before_contract_creation(self, block_identifier: BlockIdentifier) -> bool:
        ...
        return block_identifier < self.uniswap_v3_factory.deployed_block if isinstance(block_identifier, int) else False  # noqa: E501


class UniswapV2Oracle(UniswapOracle):

    def __init__(self, evm_inquirer: 'EvmNodeInquirer'):
        super().__init__(evm_inquirer=evm_inquirer, version=2)
        self.uniswap_v2_lp_abi = self.evm_inquirer.contracts.abi('UNISWAP_V2_LP')
        self.update_factory_contract()

    def update_factory_contract(self) -> None:
        self.uniswap_v2_factory = self.evm_inquirer.contracts.contract(UNISWAPV2_FACTORY_ADDRESSES[self.evm_inquirer.chain_id])  # noqa: E501
    ...
    def get_pool_price(
        ...
        pool_contract = EvmContract(
            ...
            deployed_block=self.uniswap_v2_factory.deployed_block,
        )
        output = self._call_methods(
            pool_contract=pool_contract,
    ...
    def is_before_contract_creation(self, block_identifier: BlockIdentifier) -> bool:
        ...
        return block_identifier < self.uniswap_v2_factory.deployed_block if isinstance(block_identifier, int) else False  # noqa: E501
