...
def query_stakedao_gauges() -> None:
    """Query StakeDAO lockers and strategies gauges from the API and cache them."""
    all_gauges: dict[ChainID, set] = defaultdict(set)

    try:  # Query lockers
        response_data = request_get_dict(url=STAKEDAO_API + '/lockers')
        ...
        log.error(f'Failed to retrieve StakeDAO lockers gauges from API due to {msg}')

    try:  # Query strategies
        for protocol in SUPPORTED_STAKEDAO_STRATEGIES:
            response_data = request_get_dict(url=STAKEDAO_API + f'/strategies/{protocol}')
        ...
        msg = f'missing key {e!s}' if isinstance(e, KeyError) else f'{e!s}'
        log.error(f'Failed to retrieve StakeDAO strategies gauges from API due to {msg}')
