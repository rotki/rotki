...
class VelodromeLikeBalances(ProtocolWithGauges):
    ...
    def query_balances(self) -> BalancesSheetType:
        ...
        if len(addresses_with_deposits := self.addresses_with_activity(event_types=self.deposit_event_types)) == 0:  # noqa: E501
            return balances

        addresses_to_token_ids = {}
        for address, events in addresses_with_deposits.items():
            addresses_to_token_ids[address] = [event.extra_data['token_id'] for event in events if event.extra_data is not None]  # noqa: E501
            ...
            for idx, result in enumerate(results):
                ...
                if (price := Inquirer().find_usd_price(self.protocol_token)) == ZERO_PRICE:
                    log.error(
                        ...
                        f"{self.counterparty} locked balances won't be accurate.",
                    )
