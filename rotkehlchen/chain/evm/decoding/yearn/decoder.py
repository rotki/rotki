...
from .constants import ETHEREUM_YEARN_PARTNER_TRACKER

if TYPE_CHECKING:
    from rotkehlchen.chain.evm.decoding.base import BaseDecoderTools
    from rotkehlchen.chain.evm.manager import EvmNodeInquirer
...
class YearnCommonDecoder(DecoderInterface, ReloadableDecoderMixin):
    def __init__(
            self,
            evm_inquirer: 'EvmNodeInquirer',
            base_tools: 'BaseDecoderTools',
            msg_aggregator: 'MessagesAggregator',
            yearn_partner_tracker: 'ChecksumEvmAddress' = ETHEREUM_YEARN_PARTNER_TRACKER,
    ) -> None:
        super().__init__(
        ...
        self.yearn_partner_tracker = yearn_partner_tracker
    ...
    def _handle_deposit_events(
        ...
        for event in events:
            if (
                event.event_type == HistoryEventType.SPEND and
                event.event_subtype == HistoryEventSubType.NONE and
                (event.address == self.yearn_partner_tracker or event.address in self.vaults[counterparty])  # noqa: E501
            ):
                ...
                deposit_event = event
                if event.address is not None and event.address != self.yearn_partner_tracker:
                    vault_token_name = _get_vault_token_name(event.address)
    ...
    def addresses_to_decoders(self) -> dict['ChecksumEvmAddress', tuple[Any, ...]]:
        ...
        ) | {self.yearn_partner_tracker: (self._decode_v2_increase_deposit,)}
