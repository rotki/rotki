...
def chunker(lst: list[Any], chunk_size: int) -> list[list[Any]]:
    """Used to chunk a list of length (N * chunksize) into a list having N sublists"""
...
class GmxBalances(ProtocolWithBalance):
    ...
    def query_balances(self) -> 'BalancesType':
        ...
        for address, events in addresses_events.items():
            ...
            unique_searches[address] = positions
        ...
        for address, positions in unique_searches.items():
        ...
        for idx, result in enumerate(call_output):  # each iteration is a different address with its positions  # noqa: E501
            ...
            for position_idx, pos_result in enumerate(chunker(pos_information[0], 9)):
                # each position has 9 values returned by gmx
                try:
                ...
                asset_price = Inquirer.find_usd_price(asset=collateral_asset)
                if asset_price == ZERO:
                    continue
                ...
                asset_amount = position_collateral_value / asset_price
