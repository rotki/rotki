...
from rotkehlchen.types import StarknetAddress, TimestampMS
from rotkehlchen.utils.misc import timestamp_to_date, ts_ms_to_sec

from .starknet_event import StarknetEvent
...
class StarknetSwapEvent(StarknetEvent, SwapEvent):
    ...
    def entry_type(self) -> HistoryBaseEntryType:
        return HistoryBaseEntryType.STARKNET_SWAP_EVENT

    @classmethod
    def deserialize_from_db(cls: type['StarknetSwapEvent'], entry: tuple) -> 'StarknetSwapEvent':
        """Deserialize a StarknetSwapEvent DB tuple.
        May raise:
        - DeserializationError
        - UnknownAsset
        """
        entry = cast('CHAIN_EVENT_DB_TUPLE_READ', entry)
        amount = deserialize_fval(entry[7], 'amount', 'starknet swap event')
        return cls(
            identifier=entry[0],
            ...
            tx_ref=entry[13].decode('utf-8') if isinstance(entry[13], bytes) else str(entry[13]),
            counterparty=entry[14],
            address=StarknetAddress(entry[15]) if entry[15] is not None else None,
        )

    def serialize(self) -> dict[str, Any]:
        return StarknetEvent.serialize(self)

    @classmethod
    def deserialize(cls: type['StarknetSwapEvent'], data: dict[str, Any]) -> 'StarknetSwapEvent':
        swap_data = cls._deserialize_swap_data(cls._deserialize_base_history_data(data))
        swap_data.pop('location')  # type: ignore[misc]
        try:
            return cls(  # type: ignore[misc]
                **swap_data,
                tx_ref=data['tx_ref'],
                counterparty=deserialize_optional(data['counterparty'], str),
                address=StarknetAddress(data['address']) if data.get('address') is not None else None,  # noqa: E501
            )
        except KeyError as e:
            raise DeserializationError(f'Did not find key {e!s} in StarknetSwapEvent data') from e
    ...
    def __repr__(self) -> str:
        ...
        return f'StarknetSwapEvent({", ".join(fields)})'

    def __str__(self) -> str:
        return (
            f'{self.event_subtype} StarknetSwapEvent in {self.location} with '
