...
class SwapEvent(HistoryBaseEntry):
    ...
    def __init__(
        ...
        'sequence_index': The event's sequence index. Optional, if not provided it will be set
        based on the event_subtype (Only works for FEE events if there is only one fee).

        """
        super().__init__(
            group_identifier=group_identifier,
            sequence_index=sequence_index if sequence_index is not None else {
...
def create_swap_events(
    ...
    """Wrapper for create_swap_events_multi_fee for when there is only one fee."""
    return create_swap_events_multi_fee(
        ...
        receive=receive,
        group_identifier=group_identifier,
        fees=[(fee, fee_notes, fee_identifier)] if fee is not None else None,
        location_label=location_label,
...
def create_swap_events_multi_fee(
        ...
        receive: AssetAmount,
        group_identifier: str,
        fees: Sequence[tuple[AssetAmount, str | None, int | None]] | None = None,
        location_label: str | None = None,
        spend_notes: str | None = None,
        receive_notes: str | None = None,
        identifier: int | None = None,
    ...
    """Create spend, receive, and optionally multiple fee SwapEvents for a single trade.
    ...
    sequence_index = 2  # fees always start at 2 after the spend (0) and receive (1)
    for fee_asset_amount, fee_notes, fee_identifier in fees:
        if fee_asset_amount.amount == ZERO:
            continue
