...
definePage({
  meta: {
    nav: { labelKey: msg.$t('navigation_menu.settings_sub.chains'), icon: 'lu-link', parent: '/settings' },
...
const navigation = computed<{ id: string; label: string }[]>(() => [
  { id: SettingsCategoryIds.CHAIN_QUERIES, label: t('chain_settings.chain_queries.title') },
...
<template>
  ...
  >
    <SettingCategory :id="SettingsCategoryIds.CHAIN_QUERIES">
      <template #title>
        {{ t('chain_settings.chain_queries.title') }}
      </template>
      <template #subtitle>
        {{ t('chain_settings.chain_queries.subtitle') }}
      </template>
