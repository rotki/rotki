...
const PAST_THE_EXPIRY_MS = 5000;
const PAST_THE_BACKOFF_MS = 5000;
...
describe('createItemCache', () => {
  ...
  describe('storage injection', () => {
    ...
    it('should keep resolved values when a new cache binds to the same storage', async () => {
      ...
      const populating = createItemCache(fetch, { storage });
      populating.resolve('KEY');
      vi.advanceTimersByTime(1000);
      await flushPromises();

      expect(populating.resolve('KEY')).toBe('value');
      expect(calls).toHaveLength(1);

      const bindingToTheSameStorage = createItemCache(fetch, { storage });
      expect(bindingToTheSameStorage.resolve('KEY')).toBe('value');
    ...
    it('should preserve the unknown (negative) cache across re-init', async () => {
      ...
      const bindingToTheSameStorage = createItemCache(fetch, { storage });
      expect(bindingToTheSameStorage.resolve('KEY')).toBeNull();
      vi.advanceTimersByTime(1000);
      await flushPromises();
  ...
  describe('deleteCacheKey LRU index', () => {
    it('should not leak the key in the LRU index, whose phantom entries would inflate the size and evict live keys early', async () => {
      const storage = createItemCacheStorage<string>();
      const { fetch } = createMockFetch({ KEY: 'value' });
  ...
  describe('resilient capacity', () => {
    ...
    it('should reclaim expired entries before growing further', async () => {
      ...
      vi.advanceTimersByTime(PAST_THE_EXPIRY_MS);
    ...
    it('should force-evict the oldest live entry and warn at the hard cap', async () => {
      ...
      const { fetch } = createMockFetch({ A: 'a', B: 'b', C: 'c', D: 'd', E: 'e' });
      const nothingEverExpires = 1_000_000;
      const { cache, resolve, size } = createItemCache(fetch, { expiry: nothingEverExpires, label: 'test', maxSize: 3, size: 2 });
  ...
  describe('refresh ordering', () => {
    it('should move the refreshed key to the most-recent end of the LRU, since an in-place set would break both recency and the expiry ordering the sweep relies on', async () => {
      const storage = createItemCacheStorage<string>();
  ...
  describe('error backoff', () => {
    it('should hold failed keys back before retrying', async () => {
      ...
      vi.advanceTimersByTime(PAST_THE_BACKOFF_MS);
      resolve('KEY');
      vi.advanceTimersByTime(1000);
      await flushPromises();
      expect(calls).toHaveLength(2);
  ...
  describe('fine-grained reactivity', () => {
    ...
    it('should not re-run a reader of one key when an unrelated key resolves', async () => {
      ...
      const aRunsBeforeBResolved = aRuns;
      ...
      expect(aRuns).toBe(aRunsBeforeBResolved);
      stopA();
      stopB();
    });
