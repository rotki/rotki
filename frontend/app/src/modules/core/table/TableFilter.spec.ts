...
describe('table-filter', () => {
  ...
  const createWrapper = (options: ComponentMountingOptions<typeof TableFilter> = {}): VueWrapper<InstanceType<typeof TableFilter>> => {
  ...
  const createMatchers = (startValidate: (value: string) => boolean = (): boolean => true, typeSuggestions: string[] = ['type 1', 'type 2', 'type 3']): StringSuggestionMatcher<FilterKeys, FilterKeyValues>[] => [
    {
    ...
    },
    {
      ...
      allowExclusion: true,
      multiple: true,
      suggestions: (): string[] => typeSuggestions,
  ...
  it('should filter matchers', async () => {
    ...
    await vi.advanceTimersToNextTimerAsync();
    expect(wrapper.find<HTMLInputElement>('input').element.value).toBe(`${matchers[1].key}=`);
  });

  it('should choose suggestions', async () => {
    wrapper = createWrapper({
      props: {
        matchers,
        matches: {},
    ...
    expect(wrapper.find<HTMLInputElement>('input.edit-input').element.value).toBe('');
    expect(wrapper.find<HTMLInputElement>('input:not(.edit-input)').element.value).toBe('type=');
  });

  it('should choose suggestions with exclusion', async () => {
    wrapper = createWrapper({
      props: {
        matchers,
        matches: {},
  ...
  it('should restore selection', async () => {
    wrapper = createWrapper({
      props: {
        matchers,
        matches: {
  ...
  it('should restore selection with exclusion', async () => {
    wrapper = createWrapper({
      props: {
        matchers,
        matches: {
  ...
  it('should shake and keep search when validation fails', async () => {
    ...
    expect(wrapper.find<HTMLInputElement>('input').element.value).toBe('start=invalid_value');
    ...
    const emissions = wrapper.emitted<(Record<string, unknown> | undefined)[]>('update:matches');
    const lastEmit = emissions?.at(-1)?.[0];
    expect(lastEmit?.start).toBeUndefined();
  });

  it('should apply filter when validation passes', async () => {
    const matchersWithValidation = createMatchers(value => value === 'valid');
  ...
  describe('strictMatching', () => {
    ...
    it('should reject filter when keyword does not match any suggestion with strictMatching', async () => {
      ...
      expect(wrapper.find<HTMLInputElement>('input').element.value).toBe('start=nonexistent');
    ...
    it('should apply filter when keyword matches a suggestion with strictMatching', async () => {
      const strictMatchers = createStrictMatchingMatchers();
      wrapper = createWrapper({
        props: {
          matchers: strictMatchers,
    ...
    it('should filter suggestions using getTextToken normalization with strictMatching', async () => {
      const strictMatchers = createStrictMatchingMatchers();
      wrapper = createWrapper({
        props: {
          matchers: strictMatchers,
  ...
  describe('chip grouping', () => {
    it('should group chips when more than 3 items with same key', async () => {
      wrapper = createWrapper({
        props: {
          matchers,
          matches: {
    ...
    it('should not group chips when 3 or fewer items with same key', async () => {
      wrapper = createWrapper({
        props: {
          matchers,
          matches: {
    ...
    it('should open menu when clicking overflow badge', async () => {
      wrapper = createWrapper({
        props: {
          matchers,
          matches: {
    ...
    it('should remove all items for key when close button clicked', async () => {
      wrapper = createWrapper({
        props: {
          matchers,
          matches: {
    ...
    it('should remove individual item from group menu', async () => {
      wrapper = createWrapper({
        props: {
          matchers,
          matches: {
