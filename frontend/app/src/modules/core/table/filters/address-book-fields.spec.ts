...
import {
  ...
  type Matcher,
} from '@/modules/core/table/filters/use-address-book-filter';
...
describe('toAddressBookFields', () => {
  it('should keep the wire keys the table already sends', () => {
    expect(toAddressBookFields(matchers, resolvers, t).map(field => field.key)).toStrictEqual([
      'nameSubstring',
      'address',
    ]);
  });
  ...
  it('should have both values written rather than picked', () => {
    const [name, address] = toAddressBookFields(matchers, resolvers, t);
  ...
  it('should draw an address with its avatar, shortened', () => {
    const [, address] = toAddressBookFields(matchers, resolvers, t);
