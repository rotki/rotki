import type { Matcher } from '@/modules/assets/prices/use-oracle-prices-filter';
import type { SharedFieldResolvers } from '@/modules/core/table/filters/shared/use-shared-field-resolvers';
import { describe, expect, it } from 'vitest';
import { toOraclePriceFields } from '@/modules/core/table/filters/oracle-price-fields';

const t = (key: string): string => key;

const resolvers: SharedFieldResolvers = {
  formatDate: (value: string): string => `date:${value}`,
  parseDate: (value: string): string | undefined => `ts:${value}`,
  resolveAssetChain: (value: string): string | undefined => `chainof:${value}`,
  resolveAssetSymbol: (value: string): string => `symbol:${value}`,
  resolveChainName: (value: string): string => `chain:${value}`,
  resolveHex: (value: string): string => `hex:${value}`,
  resolveLocationName: (value: string): string => `location:${value}`,
  resolveProtocolName: (value: string): string => `protocol:${value}`,
  resolveTokenName: (value: string): string => `token:${value}`,
};

const matchers: Matcher[] = [
  {
    asset: true,
    description: 'filter by source asset',
    key: 'from_asset',
    keyValue: 'fromAsset',
    suggestions: async (): Promise<string[]> => [],
  },
  {
    description: 'filter by oracle source',
    key: 'source',
    keyValue: 'sourceType',
    string: true,
    suggestions: (): string[] => ['coingecko'],
    validate: (value: string): boolean => value === 'coingecko',
  },
  {
    description: 'filter by start date',
    key: 'start',
    keyValue: 'fromTimestamp',
    string: true,
    suggestions: (): string[] => [],
    validate: (): boolean => true,
  },
  {
    description: 'filter by end date',
    key: 'end',
    keyValue: 'toTimestamp',
    string: true,
    suggestions: (): string[] => [],
    validate: (): boolean => true,
  },
];

const resolveSourceLabel = (value: string): string => (value === 'coingecko' ? 'CoinGecko' : value);

describe('toOraclePriceFields', () => {
  it('should fold the two date matchers into one period field', () => {
    const fields = toOraclePriceFields(matchers, resolvers, t, resolveSourceLabel);
    expect(fields.map(field => field.key)).toStrictEqual(['fromAsset', 'sourceType', 'period']);

    const period = fields[2];
    expect(period.valueType).toBe('date');
    expect(period.bounds).toStrictEqual({ lower: 'fromTimestamp', upper: 'toTimestamp' });
  });

  it('should show a period bound as a date rather than a timestamp', () => {
    const period = toOraclePriceFields(matchers, resolvers, t, resolveSourceLabel)[2];
    expect(period.formatBound?.('1705320000')).toBe('date:1705320000');
    // No serializer of its own: a bound is stored as the timestamp it is sent as.
    expect(period.serializer).toBeUndefined();
  });

  it('should draw an asset with its icon, symbol and chain', () => {
    const asset = toOraclePriceFields(matchers, resolvers, t, resolveSourceLabel)[0];
    expect(asset.display).toBe('asset');
    expect(asset.valueType).toBe('asset');
    expect(asset.resolveLabel?.('eip155:1/erc20:0xA0b8')).toBe('symbol:eip155:1/erc20:0xA0b8');
  });

  it('should read a source the way the row it filters to does', () => {
    const source = toOraclePriceFields(matchers, resolvers, t, resolveSourceLabel)[1];
    expect(source.label).toBe('oracle_prices.filter_field_labels.source');
    expect(source.resolveLabel?.('coingecko')).toBe('CoinGecko');
    expect(source.suggest?.()).toStrictEqual(['coingecko']);
  });
});
