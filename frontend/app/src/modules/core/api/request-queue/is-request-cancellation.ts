 ...
 * through their `AbortController`. ofetch wraps an aborted fetch in a `FetchError`
 * carrying the `AbortError` as its cause, so that shape counts too. Cancellation is expected during logout,
