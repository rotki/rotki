...
<template>
  <ServiceKeyCard
    :key-set="!!key"
    :title="t('external_services.birdeye.title')"
    :subtitle="t('external_services.birdeye.description')"
    :image-src="getPublicServiceImagePath('birdeye.png')"
  ...
  >
    ...
    <ServiceKey
      ...
      :hint="t('external_services.birdeye.hint')"
      :loading="loading"
      :status="status"
      @save="save($event)"
    >
      ...
      >
        <template #link>
          <ExternalLink
            color="primary"
            :url="externalLinks.birdeyeApiKey"
          >
            {{ t('common.here') }}
          </ExternalLink>
        </template>
