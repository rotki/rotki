...
describe('module-selector', () => {
  ...
  it('should display active modules', () => {
    expect(wrapper.find<HTMLInputElement>('[data-testid=eth2-module-switch] input').element.checked).toBe(true);
  });
  ...
  it('should disable module on click', async () => {
    ...
    expect(wrapper.find<HTMLInputElement>('[data-testid=eth2-module-switch] input').element.checked).toBe(true);
    await wrapper.find<HTMLInputElement>('[data-testid=eth2-module-switch] input').setValue(false);
    await nextTick();
    await flushPromises();
    expect(wrapper.find<HTMLInputElement>('[data-testid=eth2-module-switch] input').element.checked).toBe(false);
