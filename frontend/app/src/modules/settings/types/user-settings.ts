...
export const OtherSettings = z.object({
  // Served by its own resource, so it is absent unless a read path spliced it in; a write does not
...
const GeneralSettings = z.object({
  ...
  bridgeMatchAmountTolerance: z.string().default(Defaults.BRIDGE_MATCH_AMOUNT_TOLERANCE),
  bridgeMatchTimeRange: z.number().int().min(1).default(Defaults.BRIDGE_MATCH_TIME_RANGE),
  ...
  disabledChainQueries: z.record(z.string(), z.array(z.string())).default({}),
...
function getGeneralSettings(settings: UserSettings): GeneralSettings {
  return {
    ...
    bridgeMatchAmountTolerance: settings.bridgeMatchAmountTolerance,
    bridgeMatchTimeRange: settings.bridgeMatchTimeRange,
    ...
    disabledChainQueries: settings.disabledChainQueries,
