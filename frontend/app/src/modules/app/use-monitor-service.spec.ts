...
vi.mock('@/composables/monitor/use-monitor-watchers', () => ({
  useMonitorWatchers: vi.fn(),
}));

vi.mock('@/modules/account/use-password-confirmation', () => ({
...
vi.mock('@/modules/tasks/use-task-monitor', () => ({
...
async function loadMonitorService(): Promise<typeof import('@/modules/app/use-monitor-service')> {
  vi.doUnmock('@/modules/app/use-monitor-service');
  vi.resetModules();
  return import('@/modules/app/use-monitor-service');
}

describe('useMonitorService', () => {
  let service: ReturnType<typeof import('@/modules/app/use-monitor-service').useMonitorService>;
