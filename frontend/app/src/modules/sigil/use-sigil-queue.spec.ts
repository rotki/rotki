...
describe('use-sigil-queue', () => {
  ...
  describe('startQueue and stopQueue', () => {
    it('should set up periodic flush timer on start', async () => {
      vi.useFakeTimers();
      const { startQueue, stopQueue } = await import('@/modules/sigil/use-sigil-queue');
      startQueue();
    ...
    it('should flush leftover events from previous session on start', async () => {
      ...
      const { enqueue, startQueue, stopQueue } = await import('@/modules/sigil/use-sigil-queue');
  ...
  describe('enqueue', () => {
    it('should store entries in IndexedDB without throwing', async () => {
      const { enqueue } = await import('@/modules/sigil/use-sigil-queue');
  ...
  describe('flush', () => {
    it('should not call fetch when queue is empty', async () => {
      ...
      const { flush } = await import('@/modules/sigil/use-sigil-queue');
      await flush();
  ...
  describe('stopQueue clears DB', () => {
    it('should clear IndexedDB on stop for opt-out compliance', async () => {
      const { enqueue, stopQueue } = await import('@/modules/sigil/use-sigil-queue');
