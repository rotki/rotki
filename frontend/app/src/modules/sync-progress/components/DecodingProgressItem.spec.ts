...
describe('modules/sync-progress/components/DecodingProgressItem', () => {
  let wrapper: VueWrapper<InstanceType<typeof DecodingProgressItem>>;
  let pinia: Pinia;
  ...
  function createWrapper(item: DecodingProgress, compact = false): VueWrapper<InstanceType<typeof DecodingProgressItem>> {
    return mount(DecodingProgressItem, {
      global: {
        plugins: [pinia],
        stubs: {
          ChainIcon: {
  ...
  describe('status display', () => {
    it('should display complete status when processed >= total', () => {
      const item = createDecodingProgress(100, 100);
    ...
    it('should display pending status when processed is 0', () => {
      const item = createDecodingProgress(0, 100);
      wrapper = createWrapper(item);
    ...
    it('should display decoding status when in progress', () => {
      const item = createDecodingProgress(50, 100);
      wrapper = createWrapper(item);
  ...
  describe('chain display', () => {
    it('should display chain icon', () => {
      const item = createDecodingProgress(50, 100, 'optimism');
      wrapper = createWrapper(item);
    ...
    it('should display progress counts', () => {
      ...
      expect(wrapper.text()).toContain('sync_progress.decoding_progress');
    });
  });

  describe('visual feedback', () => {
    it('should animate for in-progress decoding', () => {
      const item = createDecodingProgress(50, 100);
      wrapper = createWrapper(item);
    ...
    it('should not animate for complete decoding', () => {
      const item = createDecodingProgress(100, 100);
      wrapper = createWrapper(item);
    ...
    it('should not animate for pending decoding', () => {
      const item = createDecodingProgress(0, 100);
      wrapper = createWrapper(item);
