 ...
 *
 * Built inside a computed so the label tracks the locale: fields built once at setup keep the
 ...
 */
export function useKrakenStakingFields(): ComputedRef<FieldDef[]> {
  ...
  return computed<FieldDef[]>(() => [toDateFieldDef({
    formatBound: dateDeserializer(dateInputFormat),
    key: 'period',
    label: t('kraken_staking_events.filter.period'),
