...
async function loadComponents(): Promise<string[]> {
  try {
    ...
    const { setupPremium } = await import('@/modules/premium/setup-premium');
    await setupPremium();
...
async function loadLibrary(): Promise<PremiumLibrary> {
  ...
  if (!library.installed) {
    ...
    const { createPremiumApi } = await import('@/modules/premium/setup-interface');
...
const PremiumLoading = defineAsyncComponent(async () => import('@/modules/premium/PremiumLoading.vue'));
const PremiumLoadingError = defineAsyncComponent(async () => import('@/modules/premium/PremiumLoadingError.vue'));
