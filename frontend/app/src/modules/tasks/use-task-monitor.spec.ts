...
describe('useTaskMonitor', () => {
  let monitor: ReturnType<typeof import('@/modules/tasks/use-task-monitor').useTaskMonitor>;
  let store: ReturnType<typeof import('@/modules/tasks/use-task-store').useTaskStore>;
  ...
  beforeEach(async () => {
    ...
    const { useTaskStore } = await import('@/modules/tasks/use-task-store');
    const { useTaskMonitor } = await import('@/modules/tasks/use-task-monitor');
    store = useTaskStore();
  ...
  it('should handle TaskNotFoundError by removing task and calling error handler', async () => {
    ...
    const { TaskNotFoundError } = await import('@/modules/tasks/types');
