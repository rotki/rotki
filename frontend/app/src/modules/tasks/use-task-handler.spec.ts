...
describe('useTaskHandler', () => {
  let handler: ReturnType<typeof import('@/modules/tasks/use-task-handler').useTaskHandler>;
  ...
  beforeEach(async () => {
    ...
    const { useTaskHandler } = await import('@/modules/tasks/use-task-handler');
    store = useTaskStore();
  ...
  describe('cancelTask', () => {
    ...
    it('should handle TaskNotFoundError by removing task', async () => {
      ...
      const { TaskNotFoundError: TNFError } = await import('@/modules/tasks/types');
