...
import { type AssetBalanceWithPriceAndChains, bigNumberify, Zero } from '@rotki/common';
import { createTestBalance } from '@test/utils/create-data';
import { assert, describe, expect, it } from 'vitest';
import { summarizeBalances } from './summarize';

function createContext(overrides: Partial<AggregationContext> = {}): AggregationContext {
  return {
    collectionOf: (): string | undefined => undefined,
    isAssetIgnored: (): boolean => false,
    mainAssetOf: (): string | undefined => undefined,
    priceOf: () => bigNumberify(100),
    resolveIdentifier: (id: string): string => id,
    ...overrides,
  };
}

describe('summarizeBalances with chains', () => {
  it('should preserve chain data for address protocol', () => {
    const blockchain: AssetBalanceEntries = {
      ETH: {
        address: {
    ...
    const result = summarizeBalances([blockchain], createContext(), { groupCollections: false, hideIgnored: false });
  ...
  it('should aggregate chains data when grouping collections', () => {
    const blockchain: AssetBalanceEntries = {
      'asset-1': {
        address: { ...createTestBalance(1, 100), chains: { eth: createTestBalance(1, 100) } },
      },
      'asset-2': {
        address: { ...createTestBalance(2, 200), chains: { polygon: createTestBalance(2, 200) } },
      },
    };

    const context = createContext({
      collectionOf: (asset: string): string | undefined =>
        asset === 'asset-1' || asset === 'asset-2' ? 'collection-1' : undefined,
      mainAssetOf: (): string => 'asset-1',
    });

    const result = summarizeBalances([blockchain], context, { hideIgnored: false });
...
describe('summarizeBalances invariants', () => {
  ...
  const context = createContext({
    ...
    isAssetIgnored: (id: string): boolean => id === 'SPAM',
    mainAssetOf: (id: string): string | undefined => collections[id]?.main,
    priceOf: () => bigNumberify(1),
  });

  function summarize(sources: AssetBalanceEntries[], groupCollections: boolean = true): AssetBalanceWithPriceAndChains[] {
    return summarizeBalances(sources, context, { groupCollections });
  }
  ...
  function reversed(sources: AssetBalanceEntries[]): AssetBalanceEntries[] {
    return [...sources].reverse().map(assets => Object.fromEntries(Object.entries(assets).reverse()));
  }
  ...
  const exchanges: AssetBalanceEntries = {
    'BTC': { kraken: createTestBalance(1, 50000) },
    'eip155:10/erc20:0xusdc': { kraken: createTestBalance(5, 5) },
  };
  const manual: AssetBalanceEntries = {
    BTC: { kraken: { ...createTestBalance(1, 50000), containsManual: true } },
    USDC: { kraken: { ...createTestBalance(7, 7), containsManual: true } },
  };
  const sources = [blockchain, exchanges, manual];
  ...
  it('should leave ignored assets out', () => {
    expect(summarize(sources).map(row => row.asset)).not.toContain('SPAM');
  });
