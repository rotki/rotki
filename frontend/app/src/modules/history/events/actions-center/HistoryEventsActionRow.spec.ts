...
describe('modules/history/events/actions-center/HistoryEventsActionRow', () => {
  ...
  it('should keep a muted issue out of the warning treatment', () => {
    ...
    expect(wrapper.find('[data-testid=actions-center-row-unmatched-movements]').exists()).toBe(false);
    expect(wrapper.find('[data-testid=actions-center-row-internal-conflicts]').classes()).not.toContain('opacity-60');
