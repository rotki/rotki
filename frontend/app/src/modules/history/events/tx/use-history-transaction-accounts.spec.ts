...
vi.mock('@/modules/core/common/use-supported-chains', () => ({
  useSupportedChains: vi.fn(() => ({
    ...
    supportsTransactions: vi.fn((chain: string) =>
      ['eth', 'optimism', 'polygon_pos', 'arbitrum_one', 'avax', 'base', 'gnosis', 'scroll', 'binance_sc', 'hyperliquid', 'monad', 'sonic', 'robinhood', 'ink', 'linea'].includes(chain),
    ),
  })),
}));
...
describe('useHistoryTransactionAccounts', () => {
  ...
  describe('disabled chains', () => {
    ...
    it('should drop a fully disabled chain from getAllAccounts', () => {
      setDisabled({ eth: [] });

      expect(composable.getAllAccounts().every(account => account.chain !== 'eth')).toBe(true);
    });
    ...
    it('should drop only the listed addresses on a partially disabled chain', () => {
      ...
      const eth = composable.getAllAccounts().filter(account => account.chain === 'eth');
      expect(eth).toEqual([{ address: '0x71C7656EC7ab88b098defB751B7401B5f6d8976F', chain: 'eth' }]);
    });

    it('should drop a disabled chain from the per-type getters too', () => {
      setDisabled({ btc: [] });

      expect(composable.getBitcoinAccounts().every(account => account.chain !== 'btc')).toBe(true);
      expect(composable.getEvmAccounts()).toHaveLength(5);
    });

    it('should honour a chain scope and the disabled setting together', () => {
      setDisabled({ optimism: [] });

      expect(composable.getAllAccounts(['eth', 'optimism'])).toHaveLength(2);
    });
  });
});
