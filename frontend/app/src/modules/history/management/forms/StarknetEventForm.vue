...
import type { NewStarknetEventPayload, StarknetEvent } from '@/types/history/events/schemas';
...
interface HistoryEventFormProps {
  data: StandaloneEventData<StarknetEvent>;
}
...
function applyEditableData(entry: StarknetEvent) {
...
function applyGroupHeaderData(entry: StarknetEvent) {
  set(sequenceIndex, data?.nextSequenceId || '0');
...
async function save(): Promise<boolean> {
  ...
  const payload: NewStarknetEventPayload = {
    address: get(address) || null,
    amount: get(numericAmount).isNaN() ? Zero : get(numericAmount),
    asset: get(asset),
    counterparty: get(counterparty) || '',
    entryType: HistoryEventEntryType.STARKNET_EVENT,
...
<template>
  <div>
    ...
    <RuiAccordions>
      <RuiAccordion
        data-cy="starknet-event-form__advance"
        header-class="py-4"
        eager
      >
        <template #header>
