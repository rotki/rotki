...
const { runTaskMock, cancelByPrefixMock } = vi.hoisted(() => ({
  cancelByPrefixMock: vi.fn(),
  runTaskMock: vi.fn(),
}));
...
vi.mock('@/modules/task-center/use-native-task', async (importOriginal) => {
  ...
  return {
    ...actual,
    // Keep the real submission bridge; only the supersede cancel is observed.
    useNativeTask: vi.fn(() => ({ ...actual.useNativeTask(), cancelByPrefix: cancelByPrefixMock })),
  };
});
...
describe('useEvmTxAutoFill', () => {
  ...
  it('should fire lookup once all inputs are valid and apply timestamp via onResolved', async () => {
    runTaskMock.mockResolvedValue(ok(createPayload()));
  ...
  it('should mark canRetry on actionable failure and surface message in errorMessages.txRef', async () => {
    runTaskMock.mockResolvedValue(err(TaskFailed({ message: 'Temporary rpc error' })));
  ...
  it('should cancel a previous lookup before starting a new one and discard the stale result', async () => {
    ...
    runTaskMock.mockResolvedValueOnce(ok(createPayload({ timestamp: 999 })));
    ...
    expect(cancelByPrefixMock).toHaveBeenCalledTimes(2);

    // Now resolve the older task; its result must be discarded.
    resolveFirst(ok(createPayload({ timestamp: 111 })));
    await flushPromises();
  ...
  it('should preserve unrelated errorMessages keys when writing lookup errors', async () => {
    runTaskMock.mockResolvedValue(err(TaskFailed({ message: 'boom' })));
  ...
  it('should rewrite not-found backend message and disable retry', async () => {
    runTaskMock.mockResolvedValue(err(TaskFailed({ message: 'Unable to find transaction 0xaf at gnosis: Transaction 0xaf was not found on gnosis' })));
  ...
  it('retry() should re-run the lookup with current inputs', async () => {
    runTaskMock
      .mockResolvedValueOnce(err(TaskFailed({ message: 'boom' })))
      .mockResolvedValueOnce(ok(createPayload({ timestamp: 100 })));
