...
describe('modules/history/internal-tx-conflicts/internal-tx-conflicts-api', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  describe('useInternalTxConflictsApi', () => {
    describe('fetchInternalTxConflicts', () => {
      it('sends GET with correct snake_case query params', async () => {
      ...
      it('includes tx_hash filter when provided', async () => {
        let capturedUrl: URL | undefined;
      ...
      it('includes fixed and failed boolean filters', async () => {
        let capturedUrl: URL | undefined;
      ...
      it('parses response with camelCase transformation', async () => {
        server.use(
