...
describe('useUnlockSteps', () => {
  ...
  describe('loginSteps.resume', () => {
    ...
    it('should not write anything when the blob is already current', async () => {
      setupStore();
      getFrontendSettings.mockResolvedValue({ decimalSeparator: '.', scrambleMultiplier: 1.5 });
      getExchanges.mockResolvedValue([]);
    ...
    it('should store the scramble multiplier the session uses when none was stored', async () => {
      setupStore();
      getFrontendSettings.mockResolvedValue({ decimalSeparator: '.' });
      getExchanges.mockResolvedValue([]);
      getRawSettings.mockResolvedValue({});

      const steps = useUnlockSteps().loginSteps;
      await steps.resume(credentials);
      await steps.loadSession();

      const sessionMultiplier = initialize.mock.calls[0][0].frontendSettings.scrambleMultiplier;
      expect(sessionMultiplier).toBeTypeOf('number');
      expect(patchFrontendSettings).toHaveBeenCalledWith({ scrambleMultiplier: sessionMultiplier }, []);
    });
  ...
  describe('createSteps(payload)', () => {
    ...
    it('should hand the new account its frontend settings, which the create response does not carry', async () => {
      setupStore();
      runTaskResult.mockResolvedValue(ok({ exchanges: [], settings: { havePremium: false } }));
      getFrontendSettings.mockResolvedValue({ decimalSeparator: ',', scrambleMultiplier: 1.5 });

      const steps = useUnlockSteps().createSteps(payload);
      await steps.login(payload.credentials);
      await steps.loadSession();

      expect(initialize).toHaveBeenCalledWith(
        { frontendSettings: { decimalSeparator: ',', scrambleMultiplier: 1.5 }, havePremium: false },
        [],
        true,
      );
    });
