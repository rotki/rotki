...
export const useHistoryEventCounterpartyMappings = createSharedComposable(() => {
  ...
  const historyEventCounterpartiesData: Ref<ActionDataEntry[]> = asyncComputed<ActionDataEntry[]>(
  ...
  const getEventCounterpartyData = (
    ...
    computed(() => {
      ...
      if (counterparty && !isValidEthAddress(counterparty)) {
        const data = get(historyEventCounterpartiesData).find(({ matcher, identifier }: ActionDataEntry) => {
          if (matcher)
      ...
      const counterpartyAddress = get(scrambleData) ? scrambleAddress(usedLabel) : usedLabel;
  ...
  const counterparties = useArrayMap(historyEventCounterpartiesData, ({ identifier }) => identifier);

  const getCounterpartyData = (counterparty: MaybeRef<string>): ComputedRef<ActionDataEntry> =>
    computed(() => {
