...
export function useAggregatedBalances() {
  ...
  const balances = (hideIgnored = true, groupMultiChain = true) =>
  ...
  const liabilities = (hideIgnored = true) =>
    computed<AssetBalanceWithPrice[]>(() => {
  ...
  const assetPriceInfo = (identifier: MaybeRef<string>, groupMultiChain: MaybeRef<boolean> = ref(false)) =>
