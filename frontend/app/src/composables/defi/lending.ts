...
export function useDefiLending() {
  ...
  const loans = (protocols: DefiProtocol[] = []) =>
    computed<DefiLoan[]>(() => {
  ...
  const loan = (identifier?: string) =>
    computed<NullableLoan>(() => {
  ...
  const loanSummary = (protocols: DefiProtocol[] = []) =>
    computed<LoanSummary>(() => {
  ...
  const lendingBalances = (protocols: DefiProtocol[], addresses: string[]) =>
    computed<DefiBalance[]>(() => {
  ...
  const effectiveInterestRate = (protocols: DefiProtocol[], addresses: string[]) =>
    computed<string>(() => {
  ...
  const totalUsdEarned = (protocols: DefiProtocol[], addresses: string[]) =>
    computed<BigNumber>(() => {
      let total = Zero;
      const showAll = protocols.length === 0;
      const allAddresses = addresses.length === 0;
  ...
  const aggregatedLendingBalances = (protocols: DefiProtocol[], addresses: string[]) =>
    computed<BaseDefiBalance[]>(() => {
      const lendBalances = get(lendingBalances(protocols, addresses));
