...
describe('store::assets/retrieval', () => {
  let store: ReturnType<typeof useAssetInfoRetrievalStore>;
  ...
  beforeEach(() => {
    ...
    store = useAssetInfoRetrievalStore();
    api = useAssetInfoApi();
  });
  ...
  describe('getAssociatedAssetIdentifier', () => {
    test('ETH2 as ETH = true', () => {
      ...
      const result = get(store.getAssociatedAssetIdentifier('ETH2'));
  ...
  describe('fetchTokenDetails', () => {
    ...
    test('success', async () => {
      ...
      const result = await store.fetchTokenDetails(address);
  ...
  describe('assetInfo, assetName, and assetSymbol', () => {
    test('identifier falsy', () => {
      ...
      expect(get(store.assetInfo(identifier))).toEqual(null);
      expect(get(store.assetName(identifier))).toEqual('');
      expect(get(store.assetName(identifier))).toEqual('');
    });
    ...
    describe('custom asset', () => {
      ...
      afterEach(() => {
        ...
        const result = get(store.assetInfo(identifier));
        ...
        expect(get(store.assetName(identifier))).toEqual(assetName);
        expect(get(store.assetSymbol(identifier))).toEqual(assetName);
      });
    ...
    describe('asset with collection parent', () => {
      ...
      test('isCollectionParent = true', () => {
        ...
        expect(get(store.assetName(identifier))).toEqual(collectionName);
        expect(get(store.assetSymbol(identifier))).toEqual(assetSymbol);
      });

      test('isCollectionParent = false', () => {
        const result = get(store.assetInfo(identifier, true, false));
    ...
    test('asset name and symbol using fallback', () => {
      ...
      expect(get(store.assetName(identifier))).toEqual(fallbackName);
      expect(get(store.assetSymbol(identifier))).toEqual(fallbackName);
    });
  });
});
