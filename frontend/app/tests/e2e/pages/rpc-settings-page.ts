...
export class RpcSettingsPage {
  ...
  async changePassword(currentPassword: string, newPassword: string): Promise<void> {
    await this.page.locator('[data-testid=current-password-input] input').clear();
    await this.page.locator('[data-testid=current-password-input] input').fill(currentPassword);
    await this.page.locator('[data-testid=new-password-input] input').clear();
    await this.page.locator('[data-testid=new-password-input] input').fill(newPassword);
    await this.page.locator('[data-testid=confirm-password-input] input').clear();
    await this.page.locator('[data-testid=confirm-password-input] input').fill(newPassword);
  ...
  async addEthereumRPC(name: string, endpoint: string): Promise<void> {
    const addButton = this.page.locator('[data-testid=add-node]');
    await addButton.scrollIntoViewIfNeeded();
    await addButton.waitFor({ state: 'visible' });
    await addButton.click();
    await this.page.locator('[data-testid=bottom-dialog]').waitFor({ state: 'visible' });
    await this.page.locator('[data-testid=node-name] input').fill(name);
    await this.page.locator('[data-testid=node-endpoint] input').fill(endpoint);
