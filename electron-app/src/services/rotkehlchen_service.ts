...
export class RotkehlchenService {
  ...
  query_periodic_data(): Promise<PeriodicClientQueryResult> {
    return new Promise<PeriodicClientQueryResult>((resolve, reject) => {
      client.invoke(
        'query_periodic_data',
        (error: Error, res: PeriodicClientQueryResult) => {
          if (error || res == null) {
  ...
  query_otctrades(): Promise<OtcTrade[]> {
    return new Promise<OtcTrade[]>((resolve, reject) => {
      client.invoke(
        'query_otctrades',
        (error: Error, result: ActionResult<OtcTrade[]>) => {
          if (error || result == null) {
            reject(error || new NoResponseError());
          } else {
            if (!result.result) {
  ...
  get_ignored_assets(): Promise<string[]> {
    return new Promise<string[]>((resolve, reject) => {
      client.invoke(
        'get_ignored_assets',
        (error: Error, result: IgnoredAssetsResponse) => {
          if (error || result == null) {
            reject(error || new NoResponseError());
          } else {
  ...
  query_exchange_balances_async(name: string): Promise<AsyncQueryResult> {
    return new Promise<AsyncQueryResult>((resolve, reject) => {
      client.invoke(
        ...
        name,
        (error: Error, result: AsyncQueryResult) => {
          if (error || result == null) {
            reject(error || new NoResponseError());
          } else {
            resolve(result);
  ...
  query_netvalue_data(): Promise<NetvalueDataResult> {
    return new Promise<NetvalueDataResult>((resolve, reject) => {
      client.invoke(
        'query_netvalue_data',
        (error: Error, result: NetvalueDataResult) => {
          if (error || result == null) {
            reject(error || new NoResponseError());
          } else {
            resolve(result);
  ...
  query_owned_assets(): Promise<string[]> {
    return new Promise<string[]>((resolve, reject) => {
      client.invoke(
        'query_owned_assets',
        (error: Error, res: ActionResult<string[]>) => {
          if (error || res == null) {
            reject(error || new NoResponseError());
          } else {
            resolve(res.result);
  ...
  get_fiat_exchange_rates(): Promise<{
    exchange_rates: { [currency: string]: string };
  }> {
    return new Promise<{ exchange_rates: { [p: string]: string } }>(
      (resolve, reject) => {
        client.invoke(
          ...
          (
            error: Error,
            res: { exchange_rates: { [currency: string]: string } }
          ) => {
            if (error || res == null) {
              reject(error || new NoResponseError());
            } else {
  ...
  get_eth_tokens(): Promise<EthTokensResult> {
    return new Promise<EthTokensResult>((resolve, reject) => {
      client.invoke(
        'get_eth_tokens',
        (error: Error, result: EthTokensResult) => {
          if (error || result == null) {
            reject(error || new NoResponseError());
          } else {
            resolve(result);
  ...
  remove_blockchain_account(
    blockchain: string,
    account: string
  ): Promise<BlockchainAccountResult> {
    return new Promise<BlockchainAccountResult>((resolve, reject) => {
      client.invoke(
        'remove_blockchain_account',
        blockchain,
