...
export class Settings {
  ...
  private get_icon_map(): { [asset: string]: string } {
    ...
    fs.readdirSync(icon_dir).forEach(function(v) {
...
export function create_settings_ui() {
  ...
  str = form_entry(
    'Floating Precision',
    'floating_precision',
    settings.floating_precision.toString(),
    '',
    'number'
  );
  str += form_checkbox(
    'anonymized_logs_input',
    'Should logs by anonymized?',
    settings.anonymized_logs
  );
  str += form_checkbox(
    'submit_usage_analytics',
    'Should anonymous usage analytics be submitted?',
    settings.submit_usage_analytics
  );
  str += form_entry(
