...
describe('user settings', function() {
  ...
  it('should add an ETH account and view the account balance', async () => {
    ...
    await client
      .getText(
        '#ethchain_per_account_table > tbody > tr:nth-child(1) > td:nth-child(1)'
      )
      .should.eventually.equal(ethAccount);
    await client
      .getText(
        '#ethchain_per_account_table > thead > tr:nth-child(1) > th:nth-child(2)'
      )
      .should.eventually.contain(blockchain);
    await client
      .getText(
        '#blockchain_per_asset_table > tbody > tr:nth-child(1) > td:nth-child(1)'
      )
      .should.eventually.contain(blockchain);

    const assetTableColumn = client.getText(
      '#blockchain_per_asset_table_body > tr:nth-child(1) > td:nth-child(3)'
    );
  ...
  it('should add a token and see that the tokens are updated', async () => {
    ...
    await client
      .element('.ms-selection')
      .getText('span=RDN')
      .should.eventually.equal('RDN');

    await client.waitUntil(
      async () => {
        return client
          .getText(
            '#blockchain_per_asset_table > tbody > tr:nth-child(2) > td:nth-child(1)'
          )
          .should.eventually.contain('RDN');
      },
      METHOD_TIMEOUT,
      'Asset Table should contain the RDN token'
    );
  });
  ...
  it('should add a btc account and see that it is added', async () => {
    ...
    await client.waitUntil(
      async () => {
        return client
          .getText(
            '#btcchain_per_account_table > tbody > tr:nth-child(1) > td:nth-child(1)'
          )
          .should.eventually.equal(btcAccount);
      },
      METHOD_TIMEOUT,
      'btcchain_per_account_table should have the BTC account'
    );
    ...
    await client.waitUntil(
      async () => {
        return await client
          .getText(
            '#blockchain_per_asset_table > tbody > tr:nth-child(3) > td:nth-child(1)'
          )
          .should.eventually.contain(blockchain);
      },
      METHOD_TIMEOUT,
      'blockchain_per_asset_table should have a BTC entry'
    );

    const assetTableColumn = client.getText(
      '#blockchain_per_asset_table_body > tr:nth-child(3) > td:nth-child(3)'
    );
    await assetTableColumn.should.eventually.satisfy((value: string) => {
      return parseFloat(value) >= 0;
    }, 'The usd value should be equal or greater than zero');
  ...
  it('should add a fiat balance and see that it is properly added', async () => {
    ...
    await app.client.waitUntil(
      async () => {
        return client
          .getText(
            '#fiat_balances_table_body > tr:nth-child(1) > td:nth-child(1)'
          )
          .should.eventually.contain('USD');
      },
      METHOD_TIMEOUT,
      'USD should appear in the #fiat_balances_table_body'
    );

    const amountColumn = client.getText(
      '#fiat_balances_table_body > tr:nth-child(1) > td:nth-child(2)'
    );
    ...
    const usdValueColumn = client.getText(
      '#fiat_balances_table_body > tr:nth-child(1) > td:nth-child(3)'
    );
  ...
  it('should change the currency and verify that it changed', async () => {
    ...
    await app.client.waitUntil(
      async () => {
        return client
          .getText('#fiat_balances_table > thead > tr > th:last-child')
          .should.eventually.contain('EUR');
      },
      METHOD_TIMEOUT,
      'fiat_balances_table should have EUR on the header'
    );

    await client.waitUntil(
      async () => {
        return client
          .getText('#blockchain_per_asset_table > thead > tr > th:last-child')
          .should.eventually.contain('EUR');
      },
      METHOD_TIMEOUT,
      'blockchain_per_asset_table should have EUR on the header'
    );

    await client.waitUntil(
      async () => {
        return client
          .getText('#ethchain_per_account_table > thead > tr > th:nth-child(3)')
          .should.eventually.contain('EUR');
      },
      METHOD_TIMEOUT,
      'ethchain_per_account_table should have EUR on the header'
    );

    await client.waitUntil(
      async () => {
        return client
          .getText('#btcchain_per_account_table > thead > tr > th:last-child')
          .should.eventually.contain('EUR');
      },
      METHOD_TIMEOUT,
      'btcchain_per_account_table should had EUR on the header'
    );
  });
});
