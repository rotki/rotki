export class DateFormatter {
    ...
    private translations: { [directive: string]: (date: Date, locale: string) => string; } = {
        'a': (date: Date, locale: string) => date.toLocaleDateString(locale, {weekday: 'short'}),
        'A': (date: Date, locale: string) => date.toLocaleDateString(locale, {weekday: 'long'}),
        'w': (date: Date) => date.getDay().toString(),
        'y': (date: Date, locale: string) => date.toLocaleDateString(locale, {year: '2-digit'}),
        'Y': (date: Date, locale: string) => date.toLocaleDateString(locale, {year: 'numeric'}),
        'b': (date: Date, locale: string) => date.toLocaleDateString(locale, {month: 'short'}),
        'B': (date: Date, locale: string) => date.toLocaleDateString(locale, {month: 'long'}),
        'm': (date: Date, locale: string) => date.toLocaleDateString(locale, {month: '2-digit'}),
        '-m': (date: Date, locale: string) => date.toLocaleDateString(locale, {month: 'numeric'}),
        'd': (date: Date, locale: string) => date.toLocaleDateString(locale, {day: '2-digit'}),
        '-d': (date: Date, locale: string) => date.toLocaleDateString(locale, {day: 'numeric'}),
        ...
        'j': (date: Date) => DateFormatter.leftPad(DateFormatter.dayOfTheYear(date).toString(), 3),
    ...
    private static leftPad(text: string, length: number = 2, padString: string = '0'): string {
        let paddedText = text;
