...
let callbacks = [];
let tasks_map = {};
let balance_tasks = [] as Array<number>;
let BalanceStatus = Object.freeze({'start': 1, 'requested': 2, 'complete': 3});
let balance_query_status = BalanceStatus.start;

class Task {
    id: number;
    type: string;
    should_expect_callback: boolean;

    constructor (task_id: number, task_type: string, expect_callback: boolean) {
        this.id = task_id;
        this.type = task_type;
...
function add_task_dropdown(task_id: number, task_description: string) {
    let str = '<li class="task' + task_id + '"><a href="#"><div><p><strong>' + task_description + '</strong><span class="pull-right text-muted">40% Complete</span></p><div class="progress progress-striped active"><div class="progress-bar progress-bar-success" role="progressbar" aria-valuenow="40" aria-valuemin="0" aria-valuemax="100" style="width: 40%"><span class="sr-only">40% Complete (success)</span></div></div></div></a></li><li class="divider task' + task_id + '"></li>';
...
export function monitor_add_callback(task_type: string, cb: any) {
...
function monitor_tasks() {
    ...
    for (let task_id in tasks_map) {
        ...
        client.invoke('query_task_result', task.id, function (error: string, res: any) {
