...
function setup_exchange_callback(event: any) {
    ...
    let exchange_name = $('#setup_exchange').val() as string;
    if (button_type === 'Remove') {
        $.confirm({
                ...
                confirm: function () {
                    client.invoke(
                        ...
                        (error, res) => {
                            ...
                            let index = settings.connected_exchanges.indexOf(exchange_name);
                            if (index === -1) {
...
function fiat_modify_callback(event: any) {
    event.preventDefault();
    let currency = $('#fiat_type_entry').val() as string;
    ...
    client.invoke(
        ...
        (error, result) => {
            ...
            if (balance === '') {
                ...
                FIAT_BALANCES[currency] = {'amount': balance, 'usd_value': get_fiat_usd_value(currency, balance.toString())};
            }
...
function delete_blockchain_account_row(blockchain: string, row: any) {
...
function add_new_eth_tokens(tokens: Array<string>) {
...
function remove_eth_tokens(tokens: Array<string>) {
    // disable selection until the entire call is done
    disable_multiselect();
    show_loading('#eth_tokens_select');
