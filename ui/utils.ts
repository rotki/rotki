...
export function reload_table_currency_val(table: any, colnum: any) {
    table.rows().invalidate();
    $(table.column(colnum).header()).text(settings.main_currency.ticker_symbol + ' value');
    table.draw();
}

export function reload_table_currency_val_if_existing(table: any, colnum: any) {
    if (table) {
...
export function unsuggest_element(selector: any) {
    $(selector).pulsate('destroy');
    $(selector).removeAttr('style');
}

export function suggest_element(selector: any, state_to_set: string) {
