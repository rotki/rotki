...
mod tests {
    ...
    async fn test_get_assets_mappings() {
        ...
        let custom_asset = custom_results
            .get("property_123_main_street")
        ...
        let (invalid_results, invalid_collections) =
            globaldb.get_assets_mappings(&invalid_assets).await.unwrap();
        assert!(invalid_results.is_empty(), "Should not find invalid assets");
        assert!(
            invalid_collections.is_empty(),
            "Should not find collections for invalid assets"
        );
        ...
        assert!(
            mixed_results.contains_key("BTC"),
            "Should find valid asset BTC"
        );
        assert!(
            mixed_results.contains_key("EUR"),
            "Should find valid asset EUR"
        );
        assert!(
            !mixed_results.contains_key("invalid_asset"),
            "Should not find invalid asset"
        );

        // Test with empty identifiers list
        let empty_assets: Vec<String> = vec![];
        let (empty_results, empty_collections) =
            globaldb.get_assets_mappings(&empty_assets).await.unwrap();
        assert!(
