...
use crate::blockchain::{SupportedBlockchain, RpcNode};
...
impl EvmNodeInquirer {
    ...
    pub async fn update_rpc_nodes(&self) -> Result<(), String> {
        let nodes = self.globaldb.get_rpc_nodes(self.blockchain)
            .await
    ...
    pub async fn get_or_create_node_connection(&self, node: &RpcNode) -> Result<Arc<DynProvider>, String> {
        if let Some(provider) = self.provider_mapping.read().await.get(node) {
            return Ok(Arc::from(provider.clone()));
        }

        let endpoint = node.endpoint.parse()
...
impl EvmInquirerManager {
    ...
    pub async fn initialize_rpc_nodes(&self) {
        ...
        for blockchain in [
            ...
            let inquirer = inquirers.entry(blockchain)
                .or_insert_with(|| Arc::new(EvmNodeInquirer::new(blockchain, self.globaldb.clone())));

            if let Err(e) = inquirer.update_rpc_nodes().await {
                error!("Failed to update RPC nodes for {}: {}", blockchain.as_str(), e);
            }
        }
    }

    pub async fn get_or_init_inquirer(&self, blockchain: SupportedBlockchain) -> Arc<EvmNodeInquirer> {
        ...
        if let Err(e) = new_inquirer.update_rpc_nodes().await {
            error!("Failed to initialize RPC nodes for {}: {}", blockchain.as_str(), e);
        }

        let mut inquirers = self.inquirers.write().await;
        inquirers.entry(blockchain)
            .or_insert(new_inquirer)
            .clone()
    }
}