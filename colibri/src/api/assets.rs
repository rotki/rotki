...
const MAX_LEV_DISTANCE: usize = 100;
...
impl Ord for RankedSearchResult {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        let self_key = (
            u8::from(!self.is_fiat),
            u8::from(!self.is_native),
            self.lev_distance,
        );
        let other_key = (
...
fn is_native_token(identifier: &str) -> bool {
    [
        ...
        SupportedBlockchain::BitcoinCash,
        SupportedBlockchain::EthereumBeaconchain,
        SupportedBlockchain::Kusama,
        SupportedBlockchain::Polkadot,
        SupportedBlockchain::Solana,
        SupportedBlockchain::ZksyncLite,
    ...
    .iter()
    .any(|blockchain| blockchain.native_token_id() == identifier)
}
...
pub async fn search_assets_levenshtein(
    ...
    let asset_type_db = payload
        .asset_type
        .as_deref()
        .and_then(AssetType::from_name)
        .map(AssetType::serialize_for_db);
    ...
    if payload.search_nfts && has_substring {
        ...
        for nft in nfts {
            ...
            if let Some(collection_name) = nft.collection_name.as_ref() {
                lev_distance =
                    lev_distance.min(levenshtein(search_needle, &collection_name.to_lowercase()));
            }
...
mod tests {
    ...
    async fn test_search_assets_does_not_fail_when_candidates_exceed_sql_limit() {
        let state = create_test_state().await;
        {
            let conn = state.globaldb.conn.lock().await;
            for index in 0..1200 {
                let identifier = format!("bulk_asset_{index}");
                let name = format!("edgebulk asset {index}");
                conn.execute(
                    "INSERT OR IGNORE INTO assets(identifier, name, type) VALUES (?, ?, ?)",
                    rusqlite::params![identifier, name, "B"],
                )
                .unwrap();
            }
        }
